...
- Вкладка **«Задания»**: списки по фильтрам (На подтверждении / В работе / Исполнено ТСД / Проведено / Отклонено).
...
- Client block: `tsd_warehouse_tasks` (по умолчанию включён в миграции).
- Экран: список → scan HU → scan место → завершить.

### Web planner (Phase 6 — частично)

- PC web (`apps/android/tsd/pc/`, вкладка **«Задания склада»**): список пакетов, детали, создание и submit `MOVE_HU` / `ADOPT_PALLET_PLAN`, preview валидации, cancel/submit.
- Web **не** вызывает `approve`, `reject`, `confirm-execution` — это остаётся в WPF.
- Drag-and-drop карта склада — **future phase** (не в текущем MVP).
