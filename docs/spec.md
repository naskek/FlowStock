...
- **Commercial / CRM-lite** (цены, КП, DOCX/PDF, ценники, мини-CRM статусы) — изолированный модуль поверх `items`/`partners`; **не** пишет в `ledger`, **не** закрывает PRD/OUT, **не** участвует в TSD-confirmed flow. Базовая цена товара задаётся в карточке товара (WPF); кастомные группы считают цену от системной группы «Базовая цена». Подробно: [`docs/architecture/commercial-offers-and-pricing-plan.md`](architecture/commercial-offers-and-pricing-plan.md).
