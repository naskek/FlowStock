...
- Серверный MVP TSD filling model для выпуска:
  ...
  - `POST /api/orders/{orderId}/production-pallets/plan` является явным действием `Сформировать план паллет`: сервер дробит обычные строки заказа по `items.max_qty_per_hu`, учитывает `order_lines.production_pallet_group` для микс-паллет, генерирует HU через server sequence и создает только `production_pallets`/`production_pallet_lines`. Новый `PRODUCTION_RECEIPT DRAFT` и `doc_lines` при планировании не создаются;
  ...
  - `GET /api/tsd/production/filling-orders` строится по `production_pallets` заказа со статусами `PLANNED`/`PRINTED` или диагностируемым `FILLED` без ledger; открытый `DRAFT PRD` не требуется;
  - `GET /api/tsd/production/orders/{orderId}/filling-context` возвращает план паллет по заказу; TSD не создает HU и не формирует план паллет;
  ...
  - `POST /api/tsd/production/fill-pallet` принимает `hu_code` и `device_id`, находит плановую паллету, создает новый фактический `PRODUCTION_RECEIPT` только для этого HU, добавляет `doc_lines` по строкам состава, закрывает PRD, пишет `ledger` и переводит паллету в `FILLED` в одной транзакции. При ошибке close/ledger паллета остается `PLANNED`/`PRINTED`, новый PRD откатывается, `ledger` не меняется;
  - при `FlowStock:ProductionAutoCloseOnFill = true` (ветка `new-ledger-logic`, default) один scan/fill HU = один закрытый PRD для этой паллеты. Legacy-планы со старым `DRAFT PRD` читаются, но fill не закрывает весь старый draft и не наполняет соседние паллеты; фактическая строка PRD помечает заменяемую legacy `doc_line` через `replaces_line_id`. Для mixed pallet одна `production_pallets.id` может иметь несколько строк состава, поэтому scan/fill/close агрегируют component rows через grouping по паллете и item, а DTO возвращает один HU с `lines[]`;
...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - Для новой TSD-модели план паллет хранится только в `production_pallets`/`production_pallet_lines`; `PRODUCTION_RECEIPT` и `doc_lines` создаются только по факту fill конкретного HU. `POST /api/orders/{orderId}/production-pallets/plan` создает план по активному заказу явным действием; `POST /api/docs/{docId}/production-pallets/plan` остается legacy/compatibility для уже подготовленных строк PRD.
