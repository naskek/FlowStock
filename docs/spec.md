...
- Отгрузка по заказу: `OUTBOUND` может быть связан с заказом (`order_id/order_ref`).
  - Для выбора в `OUTBOUND` доступны клиентские заказы с положительным `shipment_remaining`; заказ без предыдущих отгрузок тоже должен быть доступен, если остаток к отгрузке ненулевой.
