...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - Повторный запуск `Сформировать заказ` или `Создать маркировку` идемпотентен: новая задача создается только на `max(0, required_marking_qty - existing_marking_qty)`, где `existing_marking_qty` берется только из задач `marking_order` с `source_type = PRODUCTION_NEED` или `PRODUCTION_ORDER`.
  - Старые/исторические `marking_order` без production-source не гасят новую производственную потребность по тому же GTIN.
