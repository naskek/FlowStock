...
- Для задач маркировки `status = Printed` означает, что Excel/печать по задаче сформированы, но не означает наличие реальных кодов в `marking_code`. `/api/marking/orders` возвращает счетчики `codes_total`, `codes_free`, `codes_bound`; задача с `codes_total = 0` все равно видима.
