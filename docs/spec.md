...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  - При выборе заказа автоматически подставляются строки заказа (`order_line_id`) с количеством `qty_ordered` (производственный сценарий не зависит от текущих складских остатков).
  - Для связанного с заказом выпуска не применяется ограничение `qty <= receipt_remaining` от уже закрытых PRD: оператор может выпустить новую партию под заказ повторно.
  ...
  - Если во всех auto-локациях нет свободных HU-мест, операция прерывается с ошибкой и не изменяет строки документа.
