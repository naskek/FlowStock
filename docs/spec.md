...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - Сервер является единственным источником истины для `POST /api/production-needs/create-orders`: команда заново пересчитывает отчет из БД в момент POST и игнорирует любые расчетные количества/строки, даже если клиент технически передал их в body.
