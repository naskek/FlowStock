...
- Экран `/hu` (поиск HU) не содержит поля ввода и кнопок; оператору показывается подсказка «Отсканируйте HU». Скан принимается глобальным hidden `#scanSink` через существующий scanner pipeline; экранная клавиатура и ручной ввод на этом экране не используются.
