...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - В WPF доступен отдельный read-only отчет `Потребность производства`.
    ...
    - Открытые строки производственных заказов с назначением `CUSTOMER_ORDER` уменьшают `to_close_orders_qty`, а строки с назначением `INTERNAL_STOCK` уменьшают `to_min_stock_qty`.
    - `total_to_make_qty = to_close_orders_qty + to_min_stock_qty`.
    - В WPF и PC Web доступна команда `Сформировать заказы`, которая вызывает единый server endpoint и создает только черновики.
    - Если есть обе части потребности, команда создает отдельные черновики: клиентские `CUSTOMER` для `До закрытия заказов` и внутренний `INTERNAL` для `На склад до мин.`.
    - Смешанный производственный заказ из отчета не создается.
  ...
  - Автоформирование заказов из отчета `Потребность производства` создает отдельные черновики по назначению (клиентские и внутренние), а не смешивает назначения в одном ручном заказе.
