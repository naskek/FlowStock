...
- Окно `Маркировка` содержит команду `Создать маркировку`, которая создает задачи ЧЗ по актуальной производственной потребности через `POST /api/marking/create-from-production-needs`. Старое значение БД/API `EXCEL_GENERATED` читается как `PRINTED` и не записывается новыми операциями.
  ...
  - Новая production-based задача маркировки создается на полный актуальный объем производственной потребности. Свободные старые `marking_code`, включая synthetic-коды после прошлых Excel export, не уменьшают `marking_order.requested_quantity` новой задачи.
...
- Задача маркировки считается выполненной/обеспеченной, когда `codes_total >= requested_quantity`. В активном списке `/api/marking/orders` без `include_completed` такие задачи скрываются; с `include_completed=1` отображаются как выполненные.
  - Свободные `marking_code` продолжают использоваться при автопривязке к `PRODUCTION_RECEIPT` и в close validation, но не уменьшают объем новой production-based задачи маркировки.
...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - `marking_required_qty` считается от полного производимого объема: базово `total_to_make_qty`, дополнительно открытый `INTERNAL` remaining защищает уже созданные производственные drafts от потери в расчете.
  ...
  - Свободные и привязанные `marking_code` не уменьшают `requested_quantity` новой задачи. Старые/исторические `marking_order` без production-source также не гасят новую производственную потребность по тому же GTIN.
