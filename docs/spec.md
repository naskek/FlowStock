...
- HU-разбалансировка при нулевом общем остатке (фантомные +/- HU): `POST /api/diagnostics/hu-balance/correction-draft` создаёт черновик `INVENTORY_CORRECTION` с обратными строками по каждому HU только если суммарный остаток item/location = 0 и есть одновременно положительные и отрицательные HU; иначе `ITEM_LOCATION_TOTAL_NOT_ZERO` и ручная корректировка.
