...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - Повторный запуск `Создать маркировку` идемпотентен: новая задача создается только на `max(0, required_marking_qty - existing_marking_qty)`, где покрытие берется из свободных `marking_code` по item/GTIN и активной еще не обеспеченной части `marking_order` с `source_type = PRODUCTION_NEED` или `PRODUCTION_ORDER`.
  - Использованные/привязанные к выпуску коды не гасят новую производственную потребность; старые/исторические `marking_order` без production-source также не гасят новую производственную потребность по тому же GTIN.
...
- При повторном расчете маркировки уже использованные/привязанные к `receipt_line_id` коды не покрывают новую производственную потребность. Покрытием считаются только свободные `marking_code` по товару/GTIN и недостающий объем активных задач, у которых `codes_total < requested_quantity`; новая `marking_order` создается, если свободных кодов и активного незавершенного объема не хватает.
