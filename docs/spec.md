...
- Items: список с ID + modal create/edit (`name`, `is_active`, `barcode/SKU`, `gtin`, `brand`, `volume`, `shelf life months`, `max qty per HU`, `tara`, `uom`, `item_type`, `min_stock_qty`) + вычисляемый статус ЧЗ + Excel import с preview и column mapping. Импорт каталога из Excel выполняется через WPF-парсер с автоопределением заголовков (нормализация переносов строк, пробелов, регистра и `ё/е`); GTIN читается как текст с сохранением ведущих нулей (13/14 цифр), пустые строки пропускаются без ошибок, дубликаты в базе пропускаются, дополнительные колонки файла (разрешительная документация, даты, состав и т.п.) парсятся только для preview и не сохраняются в БД. Колонка упаковки/тары необязательна: при точном или однозначном alias-совпадении (подстрока в имени тары, без учёта регистра и `ё/е`) проставляется `tara_id`, иначе товар импортируется без тары с предупреждением в preview, без row-level ERROR. Ручной выбор колонок остаётся fallback. Ручной чекбокс маркировки в карточке товара не используется.
