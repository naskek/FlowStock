...
- `order_lines(id, order_id, item_id, qty_ordered, production_pallet_group NULL)` // `production_pallet_group` — инертное legacy-поле: mixed-паллеты формирует серверный конструктор (`plan-preview`/`plan-explicit`), а не группа строк; поле не редактируется из UI, а легаси-`/plan` отклоняет непустую группу (`LEGACY_MIXED_GROUP_NOT_SUPPORTED`)
...
- Серверный MVP TSD filling model для выпуска:
  ...
  - План паллет создаётся двумя явными серверными путями: основной — конструктор (`GET .../production-pallets/plan-preview` + `POST .../production-pallets/plan-explicit`, в т.ч. mixed по equal-cap); legacy `POST /api/orders/{orderId}/production-pallets/plan` остаётся только single-item (дробит строки по `items.max_qty_per_hu`), **не** формирует mixed и отклоняет строки с непустым `production_pallet_group` ошибкой `LEGACY_MIXED_GROUP_NOT_SUPPORTED`. Оба пути создают/находят технический открытый PRD, генерируют HU через server sequence и создают `production_pallets`/`production_pallet_lines` без записи в `ledger`;
  - Scoped-синхронизация плана при сохранении/обновлении заказа является trim-only и не формирует mixed-паллеты: `production_pallet_group` — инертное legacy-поле, группа строк больше не расширяется и не создаёт общий HU. Mixed-паллеты создаются только конструктором (`plan-preview`/`plan-explicit`).
  ...
  - Микс-паллета задаётся только в серверном конструкторе паллет (`plan-explicit`, equal-cap: одинаковый положительный `items.max_qty_per_hu` у всех компонентов, сумма ≤ вместимости). Колонки `Общий HU`/`Группа HU` из карточки заказа удалены; `production_pallet_group` — инертное legacy-поле и не является источником состава;
...
- Items: список без технического ID БД + modal create/edit (`name`, `is_active`, `barcode/SKU`, `gtin`, `brand`, `volume`, `shelf life months`, `storage_conditions`, `max qty per HU`, `tara`, `uom`, `item_type`, `min_stock_qty`) + вычисляемый статус ЧЗ + Excel import с preview и column mapping. Во вкладке WPF `Каталог` список фильтруется локально по поиску (`name`, `barcode/SKU`, `gtin`, без учета регистра) и по группам `Бренд`, `Объем`, `Базовая единица`; значения внутри группы объединяются по OR, группы между собой по AND, пустые/whitespace-only значения показываются как `(пусто)`, а варианты строятся по полному загруженному каталогу и сохраняют выбранность при refresh, выбирая новые значения по умолчанию. `POST /api/items` и `POST /api/items/{id}` принимают опциональное `storage_conditions: string | null`; `GET /api/items` и `GET /api/items/by-barcode/{barcode}` возвращают `storage_conditions: string | null`. Сервер нормализует `null`, пустое и whitespace-only значение в `NULL`, а непустое значение обрезает только по краям, без схлопывания внутренних пробелов, пунктуации, `%`, `+`, `0С`, русских символов и переносов строк. Импорт каталога из Excel выполняется через WPF-парсер с автоопределением заголовков (нормализация переносов строк, пробелов, регистра и `ё/е` для заголовков и обычных текстовых полей); GTIN читается как текст с сохранением ведущих нулей (13/14 цифр), пустые строки пропускаются без ошибок, дубликаты в базе пропускаются. Колонка `Условия хранения` сохраняется в `storage_conditions` при создании новых товаров из Excel и нормализуется trim-only; дубликаты из Excel по-прежнему пропускаются и не обновляют существующие товары. Прочие дополнительные колонки файла (разрешительная документация, даты, состав и т.п.) парсятся только для preview и не сохраняются в БД. Колонка упаковки/тары необязательна: при точном или однозначном alias-совпадении (подстрока в имени тары, без учёта регистра и `ё/е`) проставляется `tara_id`, иначе товар импортируется без тары с предупреждением в preview, без row-level ERROR. Ручной выбор колонок остаётся fallback. В карточке товара подпись и поле `min_stock_qty` показываются только при `item_types.enable_min_stock_control = true`; подпись и поле `max_qty_per_hu` показываются и требуют положительное значение только при `item_types.enable_hu_distribution = true`. При сохранении существующего товара с выключенным HU-распределением WPF сохраняет прежнее `max_qty_per_hu`, не валидирует и не очищает его; для нового товара значение может быть `NULL`. Ручной чекбокс маркировки в карточке товара не используется.
...
- Incoming requests: единое WPF-окно входящих запросов (из колокольчика/меню) для item requests и web order requests, со всеми действиями обработки в одном месте.
  ...
  - Computed `READY_HU_BINDING_AVAILABLE` отображается в WPF badge/inbox как read-only сигнал готовых HU, если `/api/orders/hu-bindings/ready` возвращает `hu_count > 0`; он не поддерживает confirm/reject/dismiss. В Phase 4C details/double-click открывает WPF-окно глобальной привязки готовых HU. Окно только добавляет staged ready/free HU, `Авто` является локальным разовым подбором, а сохранение идет только через `/api/orders/{orderId}/hu-bindings/apply-final`; при partial failure успешные заказы остаются сохраненными, failed/not-run staged entries остаются видимыми до refresh.
...
- В `ACCEPTED` (UI: `Готов`) заказ `CUSTOMER` переходит только после полного комплекта по всем строкам (`produced >= qty_ordered`), и только после этого ожидает фактическую отгрузку.
  ...
  - Для строки legacy-выпуска без `production_pallets` можно включить флаг общего HU (`doc_lines.pack_single_hu`): такая строка не дробится при автораспределении и может быть уложена вместе с другими отмеченными строками на один общий HU, если суммарная загрузка паллеты по формуле `sum(qty / max_qty_per_hu)` не превышает `1`. В palletized flow действует MVP-правило конструктора (equal-cap: одинаковый положительный `max_qty_per_hu` у всех компонентов); дробная формула `sum(qty/cap) ≤ 1` остаётся только в legacy-проведении и рассматривается как возможное будущее обобщение вместимости mixed-паллеты, не MVP.
