...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - При выборе заказа автоматически подставляются только незакрытые строки заказа (`order_line_id`) с количеством `receipt_remaining`.
  ...
  - После закрытия PRD, привязанного к заказу клиента (`CUSTOMER`), заказ автоматически переходит в `ACCEPTED` (UI: `Готов к отгрузке`) до фактической отгрузки.
