...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - Та же команда дополнительно запускает server-side создание задач маркировки по маркируемым товарам на полный производимый объем `total_to_make_qty`.
  - Если `to_min_stock_qty = 0`, но `to_close_orders_qty > 0`, `INTERNAL`-черновик не создается, но задача маркировки по маркируемой клиентской производственной потребности создается.
  - В разделе `Маркировка` доступна команда `Создать маркировку`, которая вызывает тот же server-side расчет по актуальной производственной потребности.
  ...
  - Повторный запуск `Сформировать заказ` или `Создать маркировку` идемпотентен: новая задача создается только на `max(0, required_marking_qty - existing_marking_qty)`, где покрытие берется из свободных `marking_code` по item/GTIN и активной еще не обеспеченной части `marking_order` с `source_type = PRODUCTION_NEED` или `PRODUCTION_ORDER`.
