...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - В WPF доступен отдельный read-only отчет `Потребность производства`.
    ...
    - Открытые строки внутреннего производственного заказа с назначением `CUSTOMER_ORDER` уменьшают `to_close_orders_qty`, а строки с назначением `INTERNAL_STOCK` уменьшают `to_min_stock_qty`.
    - `total_to_make_qty = to_close_orders_qty + to_min_stock_qty`.
  - У строк производственного заказа и строк выпуска хранится назначение: `CUSTOMER_ORDER` (`Под заказ`) или `INTERNAL_STOCK` (`На склад`). Один товар может быть добавлен в один внутренний заказ несколькими строками, если назначение строк отличается; такие строки не схлопываются в одну.
