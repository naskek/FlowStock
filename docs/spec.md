...
- Окно `Маркировка` содержит команду `Создать маркировку`, которая создает задачи ЧЗ по актуальной производственной потребности через `POST /api/marking/create-from-production-needs`. Старое значение БД/API `EXCEL_GENERATED` читается как `PRINTED` и не записывается новыми операциями.
  - Команда `Создать маркировку` создает только `marking_order`/задачу и не создает `marking_code`, не формирует Excel и сама по себе не переводит заказ в состояние `Маркировка проведена`.
  - Сервер сам пересчитывает актуальный `required_marking_qty` в момент POST и не принимает production qty из клиента.
  - `required_marking_qty` считается как фактический оставшийся объем производства к маркировке: сервер сопоставляет текущую строку `Потребность производства` и открытый `INTERNAL` remaining, но не суммирует их безусловно как две независимые потребности.
  - Если open `INTERNAL` draft и server-side production need описывают один и тот же будущий выпуск, маркировка создается на один объем, а не на сумму двух представлений.
- Задачи маркировки по производственной потребности хранятся в `marking_order` с `order_id = NULL` и `source_type = PRODUCTION_NEED`; они не зависят напрямую от клиентского заказа.
- Очередь `Маркировка` и API `/api/marking/orders` являются списком задач маркировки, а не списком клиентских заказов. API возвращает идентификатор задачи `marking_order.id` (`marking_order_id`) для production-based и order-based задач. Production-based задачи с `order_id = NULL` и `source_type = PRODUCTION_NEED` / `PRODUCTION_ORDER` обязательно отображаются; отсутствие клиентского заказа не скрывает задачу из очереди.
...
- Excel ЧЗ формируется по выбранным задачам маркировки, а не только по клиентским заказам. Production-based `marking_order` с `order_id = NULL` является валидным источником Excel.
