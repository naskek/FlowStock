...
- Окно `Маркировка` содержит команду `Создать маркировку`, которая создает задачи ЧЗ по актуальной производственной потребности. Старое значение БД/API `EXCEL_GENERATED` читается как `PRINTED` и не записывается новыми операциями.
...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - Расчет маркировки учитывает текущую потребность, открытые незакрытые `INTERNAL`-черновики/заказы, уже созданные активные/завершенные задачи маркировки и не создает повторный объем.
  ...
  - Повторный запуск `Сформировать заказ` или `Создать маркировку` идемпотентен: новая задача создается только на `max(0, required_marking_qty - existing_marking_qty)`.
