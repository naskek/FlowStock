...
- Окно `Маркировка` показывает очередь заказов со статусами `IN_PROGRESS`/`ACCEPTED`, где есть строки ЧЗ и заказ еще не обработан (`marking_status` не `PRINTED`). Старое значение БД/API `EXCEL_GENERATED` читается как `PRINTED` и не записывается новыми операциями.
- Основной список заказов WPF показывает короткий effective-статус `Маркировка ЧЗ`: `Не требуется`, `Требуется`, `Готов к нанесению`. Effective-статус считается не только из `orders.marking_status`, но и из наличия строк заказа с ЧЗ-потребностью: `item_types.enable_marking = true`, непустой `items.gtin` и `qty_ordered - shipped_qty - reserved_qty > 0`.
...
- `marking_status`: `NOT_REQUIRED` (`Маркировка не требуется`), `REQUIRED` (`Требуется файл ЧЗ`), `PRINTED` (`ЧЗ готов к нанесению`).
...
- Выпуск продукции: приемка готовой продукции на склад (плюс в `ledger`), HU обязателен на момент проведения по каждой строке, партия производства хранится в `production_batch_no`, документ может быть связан с заказом (`order_id/order_ref`).
  ...
  - В WPF доступен отдельный read-only отчет `Потребность производства`.
    ...
    - `planned_production_qty` считается как сумма `qty_remaining` по открытым/незакрытым производственным заказам и черновикам `INTERNAL`; закрытые документы в planned-вычитание не входят, потому что уже отражены через `ledger`.
    - Planned-покрытие применяется с приоритетом: сначала `До закрытия заказов`, затем `На склад до мин.`.
    - `to_close_orders_qty = raw_to_close_orders_qty - min(raw_to_close_orders_qty, planned_production_qty)`.
    - `to_min_stock_qty = max(0, raw_to_min_stock_qty - max(0, planned_production_qty - min(raw_to_close_orders_qty, planned_production_qty)))`.
    ...
    - Колонка `Всего произвести` показывает `total_to_make_qty` и используется как `qty` строк создаваемого черновика.
    - Команда создает один `INTERNAL`-черновик (`status = DRAFT`, `partner_id = null`) по всем строкам, где `total_to_make_qty > 0`.
  ...
  - Автоформирование заказов из отчета `Потребность производства` создает один внутренний производственный черновик без клиента; строки такого заказа могут оставаться с `production_purpose = INTERNAL_STOCK`.
