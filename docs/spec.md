...
- Экран `/hu` (поиск HU) при открытии получает scanner focus в scan-mode (`readOnly`, `inputmode=none`) без вызова экранной клавиатуры; физический скан сразу попадает в поле поиска. Ручная клавиатура включается только после явного тапа по полю.
