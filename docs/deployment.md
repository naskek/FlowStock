...
- Номера `V0036`–`V0042` уже использовались в исторической/параллельной marking-линии `dev` и могут присутствовать в развёрнутых БД или будущей интеграции этой линии. Текущая active-main цепочка после `V0035` продолжает новые миграции с `V0043`; номера `V0036`–`V0042` повторно не использовать.
