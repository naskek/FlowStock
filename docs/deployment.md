...
- Номера `V0036`–`V0040` исторически зарезервированы: они использовались в отброшенной marking-cutover линии и могут оставаться в `schema_migrations` уже развёрнутых БД. Текущая активная цепочка после `V0035` продолжается с `V0041`; эти исторические номера повторно не использовать.
