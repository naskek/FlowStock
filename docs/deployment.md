...
Additive миграции V0038–V0040 входят в тот же обычный maintenance release и применяются тем же one-shot `migrator` только внутри канонического PowerShell deployment. V0040 создаёт schema/guards, но не формирует cohort при работающих writers; immutable frozen snapshot записывает только последующий exact-hash enforce после остановки writers. Старые synthetic approvals/retirements после V0040 являются historical-only и не входят в новый operational preflight. Для V0038–V0040 запрещены helper/legacy scripts, короткий SSH deploy, отдельный migrator deploy и ручной production SQL.
