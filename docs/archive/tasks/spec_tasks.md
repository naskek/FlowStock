...
> **Статус: deprecated / removal candidate (архив).**
>
> Документ **не является источником истины**. Актуальные правила TSD — в [`docs/spec.md`](../../spec.md) и [`docs/spec_orders.md`](../../spec_orders.md). Нормальный production/outbound flow **не** определяется Warehouse Task Board.
>
...
- [`spec.md`](../../spec.md) — общие инварианты `ledger`, документы, normal TSD `Наполнение` / `Отгрузка`.
- [`spec_orders.md`](../../spec_orders.md) — заказы, план паллет, mixed coverage, маркировка.
