...
- Selected/pre-plan adoption и legacy whole-plan `AdoptPlanFromInternal` обязаны писать одинаковый provenance shape. На этом этапе их существующая business semantics не унифицируется: selected flow по-прежнему уменьшает INTERNAL demand, legacy whole-plan flow сохраняет прежнее поведение до отдельного lifecycle slice.
