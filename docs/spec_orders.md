...
- Публичные wire types раздельны: `ProductionTaskPresentation` и `OperationalHuPresentation`; общий публичный `layer` enum не вводится. Production branch содержит только `AWAITING_FILL` / «Ожидает наполнения»; operational codes/labels: `ON_STOCK` / «На складе», `RESERVED` / «Зарезервирован», `AWAITING_SHIPMENT` / «Ожидает отгрузки», `SHIPPED` / «Отгружен», `INCONSISTENT` / «Несогласованное состояние». `PLANNED` и `PRINTED` до fill оба отображаются как `AWAITING_FILL`; печать остаётся отдельным technical/action fact.
...
- В строках заказа в WPF и PC web отображаются наименование, SKU/штрихкод и GTIN. Модальное окно деталей заказа PC web:
  ...
  - Для `CUSTOMER` canonical operational block строится только из `hu_presentation.operational_hus` и имеет колонки `HU | Кол-во | Состояние | Локация`. Shipped HU остаётся в том же списке со статусом `Отгружен`; отдельного shipped block нет.
  - Production block выводится только при непустом `hu_presentation.production_tasks`; его main status всегда `AWAITING_FILL` / «Ожидает наполнения». Raw `PRD`, `Plan/Filled`, «Привязка», «Движение HU», legacy fate/status и архитектурные термины в main UI не показываются.
  - Server `coverage.missing_qty`/shortage показывается компактно отдельно от HU state. Для `INTERNAL` сохраняется отдельная семантика итогов выпуска; legacy coverage без HU не создаёт HU presentation.
