...
В основном списке заказов WPF дополнительно показывается короткая колонка `Маркировка ЧЗ`:
...
В WPF при выборе заказа для `PRODUCTION_RECEIPT` заказ не скрывается, но в списке показывается подсказка `Маркировка ЧЗ: проведена/требуется/не требуется`; заказы со статусом ЧЗ, требующим действий, помечаются как `[ЧЗ не проведена]`.
