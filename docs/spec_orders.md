...
Физическое удаление `DRAFT`-заказа и его строк выполняет authoritative marking-history guard внутри той же транзакции после canonical `orders` row lock и до очистки receipt plan/строк. Immutable requests/scopes/imports/codes, marking audits/cohort lineage и export batches не удаляются каскадно и не переписываются ради cleanup. Заказ с такой историей возвращает `ORDER_MARKING_HISTORY_DELETE_FORBIDDEN`: оператор должен отменить заказ и сохранить его как историю. Если update/save исключает отдельную строку с marking history, сервер сохраняет её и выставляет `cancelled_at`/server-derived cancel metadata; физический low-level delete такой строки fail-closed возвращает `ORDER_LINE_MARKING_HISTORY_DELETE_FORBIDDEN`. Строка без historical/runtime dependencies по-прежнему удаляется физически.
...
- `pallet_no` INTEGER NOT NULL DEFAULT 0 // persisted PRD-local projection для correction/legacy semantics; не authority физической этикетки
- `pallet_count` INTEGER NOT NULL DEFAULT 0 // persisted PRD-local projection для correction/legacy semantics; не authority физической этикетки
...
- После сохранения WPF/API сервер выполняет quantity-aware reconcile уже существующего future production plan. `ProductionFuturePlanTargetCalculator` сначала получает единственный target: для `CUSTOMER` — ordered quantity минус canonical deduplicated warehouse/shipped/filled protected coverage, для `INTERNAL` — ordered quantity минус canonical confirmed receipt/filled coverage. Активные `PLANNED/PRINTED` паллеты в protected coverage не входят: они являются объектом reconcile и вычитаются из target ровно один раз.
- Reconcile обычной single-item строки сохраняет identity pallet/HU/component/marking subject. При decrease surplus снимается с конца: boundary HU уменьшается in-place, а целые trailing `PLANNED` HU отменяются только когда их количество целиком больше не нужно. При increase сначала используется свободная `max_qty_per_hu` capacity последней совместимой HU, затем delta дробится на новые HU в том же active DRAFT PRD, где находится boundary/current plan. Более новый посторонний DRAFT PRD не выбирается. Первый PRD/HU по строке по-прежнему создаётся только явной командой PlanOrder; отвязка warehouse HU не восстанавливает plan автоматически.
- Partial resize `PRINTED` HU разрешён с тем же pallet id/HU, но любая смена фактического server-owned payload физической этикетки делает её `REPRINT_REQUIRED`. Автоматическое полное удаление `PRINTED` HU запрещено; это только explicit WPF delete-plan workflow с предупреждением. Удаление соседней HU может изменить динамические `pallet_no/pallet_count` surviving PRINTED labels, поэтому сервер возвращает их HU для обязательной перепечати.
- Canonical `fingerprint-v1` — SHA-256 точного server-owned payload, который WPF передаёт в BarTender: HU, item/name/brand/storage conditions, qty/uom, order/client/PRD refs, динамические order-wide `pallet_no/pallet_count`, storage place, comment, mixed flag/composition и три materialized component name/qty slots. `ProductionDate` и `BatchNumber` вводятся оператором в print session и не входят в persisted fingerprint. Fingerprint вычисляется централизованно из `ProductionPalletPrintRow`; mutation paths не увеличивают ручной revision counter.
- Physical `PalletNo/PalletCount` для production label вычисляются только по всем printable production pallets текущего заказа (`PLANNED|PRINTED|FILLED`, без `CANCELLED`) в ascending `production_pallets.id`, независимо от PRD grouping; `ReservedHu` в этот count не входят. Та же order-wide последовательность используется `GetProductionPalletPrintRows`, fingerprint, `reprint_required` и mark-printed verification. Persisted `production_pallets.pallet_no/pallet_count` остаются отдельной PRD-local correction/legacy projection (`RecalculateProductionPalletNumbers(prd_doc_id)`) и никогда не сравниваются с physical numbering. Adoption сохраняет pallet ID; появление HU в target order поэтому детерминированно меняет order-wide numbering/count и инвалидирует затронутые ранее подтверждённые labels через fingerprint.
- `printed_at IS NULL` означает отсутствие server-confirmed physical print и требует NULL fingerprint/version в normal runtime, но V0042 не переписывает historical anomalies ради этого lifecycle-инварианта. Historical `PRINTED` с `printed_at`, но NULL fingerprint/version — `LEGACY_UNVERIFIED`; `PRINTED + NULL printed_at`, `PLANNED + printed_at` и inconsistent fingerprint pair также fail-closed и попадают в maintenance blockers. Verified evidence имеет обе non-NULL колонки, version `1`, 64 lowercase hex chars и `printed_at`. `CURRENT` означает equality printed/current fingerprint, `REPRINT_REQUIRED` — mismatch. И `LEGACY_UNVERIFIED`, и `REPRINT_REQUIRED` возвращают `reprint_required=true`, поэтому WPF выбирает их для перепечати по умолчанию. No-op reconcile не меняет current fingerprint и не записывает print evidence.
- Controlled `maintenance production-label-fingerprint-backfill` использует тот же canonical production print-row builder/fingerprint: dry-run ничего не пишет, apply требует `--confirm APPLY`, блокирует заказы и атомарно grandfather-ит текущий payload только для `PRINTED + printed_at + NULL fingerprint/version` активного заказа и DRAFT PRD, для которых можно построить canonical print row. Отчёт содержит order/pallet/HU/status, наличие `printed_at`, current fingerprint, action и blocker. Lifecycle anomalies, terminal orders и non-DRAFT PRD не исправляются; повторный apply идемпотентен. Команда не меняет `ledger`, marking provenance, документы, строки заказов или pallet status/timestamp.
- Production print GET требует capability header `X-FlowStock-Production-Label-Contract: fingerprint-v1`. Acknowledgement принимает только `pallets[{pallet_id, expected_label_fingerprint}]`, повторно вычисляет payload под order lock и при mismatch возвращает `409 PRODUCTION_PALLET_LABEL_STALE`. Legacy bare `pallet_ids` всегда получает `426 WPF_PRODUCTION_LABEL_UPGRADE_REQUIRED`: Server не может доказать, какой payload старый WPF физически напечатал. Reserved warehouse HU не имеют production fingerprint и продолжают печататься независимо; без capability Server может вернуть только `ReservedHu` rows.
- Mixed HU, component progress, FILLED/ledger-backed pallet, CLOSED PRD и inconsistent lineage автоматически не resize-ятся. Конфликт откатывает вместе order quantity и весь reconcile; normal edit не пишет `ledger` и не изменяет CLOSED documents.
...
- При увеличении количества уже palletized `INTERNAL`-строки обычное сохранение продолжает существующий future plan по правилам capacity/boundary выше. Свободные складские HU не считаются INTERNAL future-production coverage; confirmed receipt ledger и FILLED production учитываются canonical helper-ами.
...
- `PRINTED` в `adopt_internal_then_plan` означает уже напечатанный физический ярлык. Перенос сохраняет pallet/HU, `PRINTED`, `printed_at` и printed fingerprint, но меняет как минимум order/client/PRD ownership payload; поэтому current fingerprint расходится и HU возвращается в `reprint_required_hus`. До перепечати TSD scan/fill fail-closed. `PLANNED` при переносе остаётся `PLANNED` и не требует перепечати, поскольку физической этикетки ещё нет.
...
- Один server-owned pallet eligibility evaluator используется в TSD filling context/picker и в authoritative fill command. Для каждого applicable component требуется exact stable subject и `active REAL_IMPORT coverage + active legacy subject exemption >= planned_qty`; non-applicable component отдельно разрешён. Read-model исключает uncovered HU и возвращает stable blocker, а command повторяет тот же predicate под transaction locks. Mixed pallet проходит только при полном покрытии каждого applicable component.
- Canonical operational request snapshot использует immutable request/scope provenance и current `marking_request_scope_consumption.active_quantity`. `EXACT_ACTIVE`, `PARTIALLY_RETIRED`, `FULLY_RETIRED` — server classifications: fully-retired request исключён из нового Excel/import/current progress; partially-retired request имеет operational required, равный active remainder, и не экспортирует исходный `requested_quantity` повторно. Его imported codes остаются у исходного request; coverage cap равен active remainder, excess non-reusable, а subsequent increase создаёт отдельный delta request с reserve ровно один раз.
- Preview и export используют один snapshot calculation. `GET /api/orders/{id}/marking/preview` возвращает `snapshot_hash` и `new_requests(required_qty,reserve_qty,requested_qty)`; `POST .../marking/export` принимает `expected_snapshot_hash`. Drift preview→export даёт `409 MARKING_EXPORT_SNAPSHOT_CHANGED` и zero writes. V0041 `marking_request_export_batch`/`..._request` неизменяемо фиксирует request membership committed workbook: exact retry исходного pre-export hash при неизменном post-export snapshot регенерирует те же rows без нового request. Имя, GTIN и количество replay-строки берутся только из `item_name_snapshot`, `gtin_snapshot` и `requested_quantity_snapshot`; текущие `items`, `item_types` и `marking_order` не являются источником содержимого replay. XLSX bytes и КМ в batch не хранятся.
- Все positive-active requests одного normalized GTIN в current related subject graph образуют единый import envelope. Confirm сериализуется с export, replan/decrease/cancel и marking reconciliation через lock соответствующего `orders` row; порядок внутри Confirm: order, затем полный current set `marking_order` по детерминированному ID. После order lock сервер заново получает envelope всех GTIN импортируемого batch и требует exact совпадения IDs/quantities/imported/scope snapshots с preview: появление, исчезновение, retirement или изменение любого request возвращает `MARKING_IMPORT_SNAPSHOT_CHANGED` до записей. Codes сортируются server-side по file hash/row/code hash; requests — по immutable creation/request identity. Первый проход закрывает `max(0, active_scoped_qty - imported_real_qty)` каждого request, второй заполняет только оставшуюся immutable reserve capacity. Поэтому increase до/после partial import, required-complete/reserve-short и несколько delta requests не создают normal `AMBIGUOUS_REQUEST_SCOPE`; этот blocker остаётся только для structural ownership/GTIN conflicts. Fully-retired historical requests в envelope не входят.
- Order-line read model возвращает `marking_progress`: applicability/configuration error, `real_required_qty`, `valid_real_covered_qty`, `active_scoped_qty` и request-level imported progress. Состояния `NOT_REQUIRED | REQUEST_REQUIRED | WAITING_FOR_CODES | PARTIAL_IMPORT | COMPLETE | ACTION_REQUIRED` формирует сервер; `PARTIAL_IMPORT` не является физическим/application status. WPF показывает server label и quantities без локального пересчёта.
- `marking_settings.default_reserve_quantity` меняется только admin endpoint, принимает любое `Int32 >= 0`, проверяет checked overflow `required + reserve` и влияет только на новые requests/export batches. Existing request и committed batch immutable.
- Preflight/hash не зависят от количества/статуса synthetic codes. Coherent DRAFT PRD и `PLANNED/PRINTED` pallet с единственным exact `ACTIVE` subject входят во frozen snapshot без real KM. Полный `FILLED` output допустим с единственным exact `COMPLETED` subject: exact определяется по `production_pallet_lines`, принадлежащей текущему PRD `doc_line`, stable subject/component/pallet, current order line, item, GTIN и planned quantity. Legacy snapshot `subject.current_doc_id` и representative pallet header `order_line_id`/`item_id`/`doc_line_id` не являются component authority. Отсутствие legacy fill timestamp при terminal `FILLED` и полном `filled_qty` само по себе не является partial progress. Missing/ambiguous/mismatched component/doc-line lineage или lifecycle, partial/inconsistent filling, orphan structure, invalid quantity, missing GTIN и unsafe real/unknown provenance остаются fail-closed.
...
- Current retry и historical re-download — разные операции. Current retry через `POST .../marking/export` возвращает исходный immutable batch только когда current operational snapshot равен его `post_export_snapshot_hash`; pre-export hash также остаётся ключом безопасного retry после неизвестного HTTP outcome. Drift не маскируется и возвращает `MARKING_EXPORT_SNAPSHOT_CHANGED`.
- `GET .../marking/export-batches/latest/archive` — read-only historical recovery потерянного локального файла. Он строит workbook только из immutable batch membership, не берёт order write lock, не создаёт/не активирует requests/scopes и не возвращает retired request в operational state. Архив имеет префикс `ARCHIVE_`, отдельный лист `АРХИВ_НЕ_ОТПРАВЛЯТЬ` и WPF warning: файл может не соответствовать текущему заказу и не должен отправляться в КМ как новая заявка.
...
- Проверка уменьшения использует необрезанный `deduplicated_coverage_qty`; для `CUSTOMER` fail-closed guard включает также factual `FILLED production_pallets` без подтверждающего ledger. Cap по `qty_ordered` применяется только к расчёту `future_need`, но не к validation: `qty_ordered` нельзя опустить ниже уже существующего unconfirmed FILLED/component coverage. Такой dirty state normal reconcile не исправляет и `ledger` не дописывает. Удаление/замена строки с shipped, confirmed/FILLED/ledger production или существующим неотгруженным ready-HU binding блокируется. `PRINTED` остаётся будущим планом для partial in-place resize, но его полное автоматическое удаление запрещено и требует explicit delete-plan workflow; обычное сохранение заказа возвращает stable error `PRINTED_PALLET_EXPLICIT_REMOVAL_REQUIRED` без частичных изменений заказа, PRD или паллет.
