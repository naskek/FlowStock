...
- При уменьшении количества для `INTERNAL` защищенный минимум = `shipped_qty + filled_production_pallet_qty`; `PLANNED`/`PRINTED` без `FILLED` не считаются фактическим выпуском и не блокируют уменьшение. Для `CUSTOMER` защищенный минимум дополнительно включает уже привязанные HU из `order_receipt_plan_lines` и `FILLED` production pallets по строке. Нарушение блокируется сообщением вида `Нельзя уменьшить количество ниже уже заполненного/выпущенного объема: заполнено {qty}.`, с перечислением HU/паллет, которые мешают уменьшению.
- При уменьшении `CUSTOMER` после сохранения строки сервер оставляет отгруженные, зарезервированные и `FILLED` HU без изменений, а лишние unfilled planned production pallets строки отменяет в рамках текущего `order_id + order_line_id`; если после отмены нужен меньший остаток к производству, сервер добавляет только недостающий planned-объем по актуальному `qty_ordered`.
