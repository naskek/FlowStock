...
- В разделе `Маркировка` есть отдельная ручная кнопка `Создать маркировку`, которая вызывает `POST /api/marking/create-from-production-needs` и создает задачи маркировки по актуальной производственной потребности без дублирования уже покрытого объема.
  - Эта команда создает только `marking_order`: она не создает `marking_code`, не формирует Excel и не должна сама по себе переводить заказ в `Маркировка проведена`.
  - Новая `marking_order` создается на полный актуальный объем производственной потребности; свободные старые `marking_code` не уменьшают `requested_quantity` новой задачи.
  - Расчет маркировки учитывает текущую производственную потребность, открытые незакрытые `INTERNAL`-черновики/заказы и уже созданные задачи маркировки с production-source.
  - `marking_required_qty = полный маркируемый производимый объем`: базово `total_to_make_qty`, плюс открытый `INTERNAL` remaining, если draft уже создан и уменьшил `to_min_stock_qty` в отчете.
  - Повторное создание маркировки идемпотентно: сервер не создает дубль, если по тому же товару уже есть активная production-based `marking_order` на текущую потребность.
  - Свободные или уже привязанные `marking_code` не гасят новую production-based потребность и не уменьшают `requested_quantity`; они используются только для close validation и автопривязки к `PRODUCTION_RECEIPT`.
...
## Маркировка ЧЗ по производственной потребности
...
Маркировка привязана к производственной потребности/производимому выпуску, а не к конкретному клиентскому заказу. Клиентские заказы остаются источником спроса, но задача маркировки должна покрывать весь маркируемый объем, который реально надо произвести: и клиентскую часть, и пополнение склада до минимума.

`marking_order` для производственной потребности может иметь `order_id = NULL`. Связь с источником задается через `source_type`: `PRODUCTION_NEED` для агрегированной производственной потребности и `PRODUCTION_ORDER` для привязки к конкретному производственному заказу/draft. Кнопка `Создать маркировку` не ищет клиентские заказы напрямую, а использует server-side расчет производственной потребности и открытого `INTERNAL` remaining.
...
Новая production-based `marking_order` создается на полный актуальный объем производственной потребности. Уже использованные/привязанные коды не гасят новую потребность, а свободные коды не уменьшают `requested_quantity`; от дублей защищают только уже существующие активные production-based задачи по тому же товару.
...
Для внутренних производственных заказов и draft, созданных из `Потребность производства`, наличие `order_receipt_plan_lines` или самой задачи `marking_order` не должно маскировать отсутствие кодов: до Excel/synthetic codes effective-статус остается `REQUIRED`.
...
Очередь WPF `Маркировка` по умолчанию выбирает и показывает задачи маркировки, а не только клиентские заказы. Production-based `marking_order` с `order_id = NULL` отображается как обычная задача; для старых order-based строк источник остается клиентским заказом. Legacy order-candidates в очереди по умолчанию ограничены активными заказами:
...
Расчет задач маркировки из производственной потребности:
...
- открытые `INTERNAL` drafts/orders добавляются к расчету, чтобы уже созданный draft не исчезал из маркировки после planned-вычитания;
...
Для `INTERNAL`:
- `produced_qty` = сумма `doc_lines.qty` по закрытым PRD, привязанным к заказу либо через `doc_lines.order_line_id = order_lines.id`, либо fallback через `docs.order_id = order.id` + совпадение по `item_id`, если строка выпуска сохранена без `order_line_id`
...
- Для `INTERNAL`:
  ...
  - пока выпуск не начат, такой `DRAFT` остается `DRAFT`
  - первый частичный закрытый `PRODUCTION_RECEIPT` переводит `DRAFT` в `IN_PROGRESS` (UI: `В работе`)
