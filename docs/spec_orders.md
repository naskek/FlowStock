...
- `CUSTOMER`
  ...
  - в TSD picker отгрузки попадают только клиентские `ACCEPTED` (`Готов`) заказы, по которым есть ожидаемые HU к отгрузке: как из `order_receipt_plan_lines` (customer-bound HU с положительным `ledger`), так и из `production_pallets` со статусом `FILLED` для этого заказа (паллетизированный выпуск) с положительным текущим HU/item stock и остатком к отгрузке по строке
