...
- В WPF разрешена смена типа сохраненного заказа только `CUSTOMER -> INTERNAL` и только если по заказу еще нет отгрузок/связанных OUTBOUND-документов.
...
- Для `CUSTOMER`:
  ...
  - `DRAFT` используется только локально в WPF для нового, еще не сохраненного заказа
  - после выпуска PRD (с привязкой к заказу) статус автоматически становится `ACCEPTED` (UI: `Готов к отгрузке`)
