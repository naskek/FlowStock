...
- Единый UI-путь покрытия для `CUSTOMER` перед планированием — окно `PrePlanCoverageDialog` из карточки заказа (кнопка «План паллет (производство)») + режим `apply_selected_coverage_then_plan`. Прежний параллельный ready-HU UI-flow (per-order кнопка «Привязка HU» и Event Center «Готовые HU» / глобальное окно привязки) удалён, чтобы не было двух конкурирующих способов привязки складских HU. Серверная ready-HU семантика (`OrderHuBindingApplyFinalService`, `HuBindingApplyShared`, read-model `/api/orders/hu-bindings/ready`) сохраняется как переиспользуемый helper unified writer, а не как отдельный клиентский `apply-final` вызов. Инлайн-резерв складских HU по строке заказа (picker в карточке) — отдельный механизм и не входит в удалённый ready-HU flow. После успешного unified apply карточка заказа поднимает `OrderStateChanged`, поэтому summary/нотификации Центра событий обновляются и stale ready-HU уведомления не остаются.
...
- Canonical server-derived fate имеет приоритет: активная строка закрытого CUSTOMER `OUTBOUND` → `SHIPPED`; действующий CUSTOMER reservation → `RESERVED`; собственная полностью готовая production HU CUSTOMER-заказа → `AWAITING_SHIPMENT`; прочий положительный ledger stock → `ON_STOCK`. Fate вычисляется при чтении и не изменяет `production_pallets.status`, `ledger` или `order_receipt_plan_lines`.
- `AWAITING_SHIPMENT` (`Ожидает отгрузки`) допустим только для persisted CUSTOMER-заказа в `IN_PROGRESS` или `ACCEPTED`, если `production_pallets.order_id` явно равен текущему заказу, каждая component `order_line_id` принадлежит этому же заказу, persisted `production_pallets.status = FILLED` и по каждому компоненту нормализованного HU есть положительный ledger balance. Для mixed HU дополнительно требуется завершение всех component lines. `EffectiveStatus` является presentation/derived значением и не заменяет persisted status как условие этого fate. `docs.order_id` сохраняется для legacy display, но не заменяет отсутствующий pallet owner при допуске нового fate. `DRAFT`, `SHIPPED`, `CANCELLED` и legacy/неконсистентный `MERGED` этот fate не получают.
- Для mixed HU все component lines должны быть завершены. Partial progress из `production_pallet_lines.filled_qty` не является готовым stock. Допуск `AWAITING_SHIPMENT` атомарен для всей pallet-группы: shipment или reservation любого component key подавляет awaiting у остальных. При этом существующие `SHIPPED`/`RESERVED` остаются component-specific и не копируются на компоненты без соответствующего candidate; `fate_qty` остаётся component-scoped.
- В source-заказе активный CUSTOMER-резерв, включая reservation этого же заказа, сохраняет существующий приоритет над `AWAITING_SHIPMENT`. Конфликт ownership `production_pallets` и `order_receipt_plan_lines` не маскируется новым fate: применяется существующий fail-closed owner/reservation resolution.
- Глобальный TSD HU resolver переиспользует только eligibility `AWAITING_SHIPMENT`, не component-specific classifier. Его приоритет остаётся operational: draft OUT → `OUTBOUND_PICKED`, active reservation → `OUTBOUND_EXPECTED`, отсутствие stock всей HU и active closed OUT line → `SHIPPED`, затем eligible `AWAITING_SHIPMENT`, затем persisted `FILLED_PRODUCTION_PALLET`. Поэтому closed shipment одного mixed-компонента блокирует awaiting, но при положительном ledger balance других компонентов не делает всю HU глобально `SHIPPED`.
- Scoped resolver SQL получает eligibility facts одним command для точного normalized HU. Ledger balance каждого уникального component key считается только из `ledger`; повторяющиеся component rows не дублируют balance. Reservation/shipment blockers line-linked, а superseded `doc_lines`, на которые ссылается `replaces_line_id`, blocker не создают. Эти facts являются server-only и не добавляются в JSON карточки.
- Любая qualifying active строка закрытого CUSTOMER `OUTBOUND` сохраняет исторический `SHIPPED`, включая частичную отгрузку при положительном остатке и несколько закрытых OUTBOUND. Используются только line-linked, не superseded `doc_lines`; наиболее поздний документ выбирается по `closed_at`, `created_at`, `doc_id`. Новая quantity-splitting модель не вводится.
...
- Для mixed production pallet source определяется по `production_pallet_lines`. Если source и target совпадают, лишняя стрелка подавляется. WPF merge заменяет bare `наполнено` canonical fate-строкой, поэтому для одного HU не показываются одновременно `Наполнено` и `Ожидает отгрузки`.
- `production_hu_rows.fate_code/fate_label/fate_qty` являются wire-контрактом PC Web и основного TSD. Клиенты только выбирают server label как основной display status для `AWAITING_SHIPMENT` и не вычисляют его условия. Fate code является additive string; неизвестный code не должен ломать старый клиент, а `fate_label` служит display fallback. Outbound TSD использует отдельный outbound read-model и этот fate-контракт не потребляет.
...
- WPF показывает pre-plan диалог при `has_warning = true` **или** `has_free_warehouse_hu = true`. Действия оператора располагаются вертикальным блоком: `Сначала привязать складские HU…` (только открывает существующее окно `Привязка HU` / apply-final, без скрытого auto-bind и без создания плана), `Перенести и допланировать` (если есть projected adoption), `План без переноса`, `Только безопасные` (только при warning), `Отмена` (не вызывает `POST /plan`). После привязки HU выполняется свежий preview до любого планирования: при нулевой нехватке производственное планирование не вызывается (UI показывает «планирование не требуется»); при остаточной нехватке без warning/free-HU условий выполняется обычное полное планирование по исходной команде оператора. Если preview недоступен из-за ошибки сервера/сети (кроме HTTP 404 старого сервера без endpoint — тогда лог и продолжение), WPF показывает нейтральное подтверждение `Не удалось проверить складские HU и ожидаемый INTERNAL-выпуск` с кнопками `План без переноса` / `Отмена`.
...
- В строках заказа в WPF и PC web отображаются наименование, SKU/штрихкод и GTIN. Модальное окно деталей заказа PC web:
  ...
  - Раскрытие строки read-only: показывает production pallets/HU. Для `CUSTOMER` секция `HU по строке заказа` объединяет текущие явно привязанные ledger-HU и уже отгруженные HU строки; текущая складская привязка и проведённая отгрузка остаются разными состояниями, при частичной отгрузке один HU может отображаться отдельными строками с текущим и отгруженным количеством. Точный серверный итог покрытия показывается при его наличии.
  - `Движение HU` (fate): для production HU отдельно показывается WPF-compatible fate read-model — текущий stock, активный резерв другого заказа или наиболее поздняя проведённая CUSTOMER-отгрузка; передача произведённого HU в другой CUSTOMER-заказ отображается как `Передано в заказ N`. Fate-данные display-only и не участвуют в coverage.
  - Только для `CUSTOMER` секция `Отгрузка этой строки заказа` показывает OUTBOUND текущей строки и не смешивается с движением HU, произведённого исходным INTERNAL/CUSTOMER-заказом. Для `INTERNAL` CUSTOMER-секция `HU по строке заказа` и секция отгрузки не показываются; серверные значения coverage отображаются как `Итог выпуска`, `Выпущено`, `Осталось выпустить`; клиентские формулировки HU binding скрыты.
