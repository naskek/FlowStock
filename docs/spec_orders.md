...
- После переноса сервер пересчитывает `order_receipt_plan_lines` внутреннего заказа и глобально пересчитывает резервы всех активных клиентских заказов с `bind_reserved_stock = true` (FIFO), чтобы выпущенный объем мог быть зарезервирован под клиентский заказ.
