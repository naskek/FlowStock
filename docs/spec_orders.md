...
- WPF показывает pre-plan диалог при `has_warning = true` **или** `has_free_warehouse_hu = true`. Действия оператора: `Сначала привязать складские HU…` (только открывает существующее окно `Привязка HU` / apply-final, без скрытого auto-bind и без создания плана), `Сформировать только позиции без предупреждения` (только при warning), `Всё равно сформировать всё`, `Отмена` (не вызывает `POST /plan`). После привязки HU выполняется свежий preview до любого планирования: при нулевой нехватке производственное планирование не вызывается (UI показывает «планирование не требуется»); при остаточной нехватке без warning/free-HU условий выполняется обычное полное планирование по исходной команде оператора. Если preview недоступен из-за ошибки сервера/сети (кроме HTTP 404 старого сервера без endpoint — тогда лог и продолжение), WPF показывает нейтральное подтверждение `Не удалось проверить складские HU и ожидаемый INTERNAL-выпуск` с кнопками `Всё равно сформировать всё` / `Отмена`.
