...
- Основной workflow ЧЗ запускается из карточки заказа: WPF сначала вызывает `GET /api/orders/{orderId}/marking/preview` (без side effects), показывает таблицу товар/GTIN/qty/HU и только после подтверждения вызывает `POST /api/orders/{orderId}/marking/export`. Источником расчета являются серверные `orders` и `order_lines`, а не очередь производственной потребности.
...
- Effective-статус активного `CUSTOMER`-заказа становится `ACCEPTED` (`Готов`), если все неполностью отгруженные строки имеют `shortage = 0` по read-model готовности к отгрузке; свободные складские HU не обязаны быть заранее записаны в `order_receipt_plan_lines`.
...
- План паллет по заказу не создает `PRODUCTION_RECEIPT DRAFT` и `doc_lines`: план хранится только в `production_pallets`/`production_pallet_lines`. TSD filling-orders/context строятся по паллетам заказа и не требуют открытого PRD.
- TSD fill паллеты атомарен: один scan/fill HU создает отдельный фактический `PRODUCTION_RECEIPT`, добавляет строки только для этой паллеты, закрывает PRD, пишет положительные строки `ledger` и переводит `production_pallets.status = FILLED` в одной транзакции. Если close/ledger падает, статус паллеты откатывается, новый PRD откатывается, `ledger` не получает частичных строк.
- Legacy-планы со старым `DRAFT PRD` продолжают читаться, но fill не закрывает весь legacy draft и не трогает соседние паллеты; фактический PRD создается отдельно для выбранного HU.
- Mixed pallet хранится как один HU (`production_pallets.id`) с несколькими `production_pallet_lines`; scan/fill/close не должны строить словари только по `pallet_id` поверх component rows без группировки. Ledger при успешном fill содержит положительную строку для каждого компонента.
- Повторное сканирование уже успешно наполненной паллеты с закрытым PRD и ledger stock возвращает idempotent success/already-filled без повторной записи ledger. `FILLED` паллета без receipt ledger не считается завершенной для TSD list/context и должна оставаться диагностируемой, а не скрывать заказ.

## HU binding CUSTOMER

- Клиентский заказ привязывает HU через явное резервирование, а не через перенос строк INTERNAL → CUSTOMER.
- WPF flow должен показывать кандидаты HU в строке CUSTOMER, позволять выбрать HU чекбоксами и отправлять выбранные HU на сервер сразу при подтверждении диалога через `POST /api/orders/{orderId}/hu-reservations/apply`; подтвержденный выбор не хранится только в локальном состоянии WPF и не откладывается до «Сохранить и закрыть».
