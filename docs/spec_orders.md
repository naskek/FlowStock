...
- Отдельный отчет `Потребность производства` считается независимо от контроля минимума и не заменяет его.
  ...
  - При этом та же команда создает/дополняет задачи маркировки для маркируемых товаров на полный производимый объем `total_to_make_qty`, а не только на складскую часть.
  - Если `to_min_stock_qty = 0`, но `to_close_orders_qty > 0`, `INTERNAL`-черновик не создается, но задача маркировки по маркируемой производственной потребности создается.
  - В разделе `Маркировка` есть кнопка `Создать маркировку`, которая вызывает server endpoint и создает задачи маркировки по актуальной производственной потребности без дублирования уже покрытого объема.
