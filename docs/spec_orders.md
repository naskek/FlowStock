...
- Canonical operator-facing projection строится `HuOperatorReadModelService` из одного результата pure `HuOperatorClassifier` на normalized HU. Внутренний result имеет ровно один вариант: no current/history-only, production task либо operational HU; projectors не повторяют precedence.
- Публичные wire types раздельны: `ProductionTaskPresentation` и `OperationalHuPresentation`; общий публичный `layer` enum не вводится. Production codes: `LABEL_NOT_PRINTED`, `AWAITING_FILL`, `FILLING`, `RELEASE_NOT_POSTED`; operational codes: `AWAITING_SHIPMENT`, `RESERVED`, `ON_STOCK`, `SHIPPED`, `INCONSISTENT`. `state.label` формирует сервер.
...
- Mixed HU классифицируется атомарно целиком. Поддерживаемый mixed fill подтверждает выбранный component полностью, поэтому production component может иметь только `filled_qty ≈ 0` или `filled_qty ≈ planned_qty`; промежуточное значение и persisted `FILLED` с незавершённым component дают `INCONSISTENT`. Разные targets/locations/reservations, неполная component shipment и uncertain correction lineage также fail-closed.
...
- Canonical server contract уже реализован аддитивно: detailed order endpoint возвращает `hu_presentation`, global HU endpoints — `operator_presentation`, а релевантные production endpoints — `production_presentation`.
- Текущее compatibility-состояние consumers: WPF, PC Web и основной TSD продолжают использовать существующие `HuDisplayRows`, `warehouse_hu_rows`, `production_hu_rows`, `shipped_hu_rows`, `production_hu_rows.fate_*`, legacy `TsdHuState` и raw production fields. Эти поля временно сохранены и остаются источником текущего legacy display до отдельной consumer migration; новый server contract уже доступен, но клиенты ещё не используют его как main status.
- В текущем compatibility fate-read-model CUSTOMER-заказа-получателя резерв и проведённая отгрузка показывают достоверно найденный source-заказ (`← выпуск заказ N`). Если source установить нельзя, стрелка не выводится; legacy `CLOSED PRODUCTION_RECEIPT` без нормализованного HU и ledger по тому же `doc + item + HU` не создаёт fate-строку.
...
- Целевое состояние после consumer migration: источником main status становится `hu_presentation`/`operator_presentation`/`production_presentation`, а legacy collections/fate/state остаются compatibility-only до удаления всех старых consumers. PC, WPF и TSD используют одинаковую server-owned semantics и terminology, не реализуя precedence локально; platform layout может различаться.
- Additive read model не меняет command eligibility, и текущие command/business flows не используют presentation state как permission. Atomic single-HU outbound enforcement не считается реализованным этим контрактом. До полной миграции actionable WPF/TSD/reservation/binding/outbound flows обязателен отдельный safety gate: либо server-side atomic-HU enforcement, либо единая server-owned fail-closed validation должна отклонять `INCONSISTENT` HU на offer/accept/close boundaries. Клиентская проверка статуса не является authorization. Outbound TSD сохраняет отдельный command/eligibility read-model.
...
- В строках заказа в WPF и PC web отображаются наименование, SKU/штрихкод и GTIN. Модальное окно деталей заказа PC web:
  ...
  - Текущее compatibility-представление раскрытия продолжает использовать legacy `warehouse_hu_rows`, `production_hu_rows`, `shipped_hu_rows` и `fate_*`: существующие секции production pallets/HU, `HU по строке заказа`, `Движение HU` и `Отгрузка этой строки заказа` сохраняют прежнее поведение. Для `CUSTOMER` текущая складская привязка и проведённая отгрузка могут отображаться отдельными строками/секциями; для `INTERNAL` сохраняются существующие итоги выпуска и скрытие CUSTOMER-specific секций.
  - Целевое представление после consumer migration использует два условных semantic блока: production progress (`HU | План/прогресс | Состояние`) и operational HU (`HU | Кол-во | Состояние | Локация`). Пустые блоки не выводятся; одна HU не может одновременно иметь два главных статуса, но разные HU одной строки могут находиться в разных блоках.
  - В целевом представлении отдельные main-level секции `Движение HU` и `Отгрузка этой строки заказа`, raw PRD/pallet/fate statuses и архитектурные термины не показываются. Shipped HU остаётся в normal operational list со статусом `Отгружен`; history/diagnostics доступны отдельно. DRAFT OUTBOUND не становится dominant status, а client-side precedence не вводится.
  - После consumer migration server `coverage.missing_qty`/shortage показывается компактно отдельно от HU state. Для `INTERNAL` сохраняется отдельная семантика итогов выпуска; legacy coverage без HU не создаёт HU presentation.
