...
- Approval не доверяет проверке времени создания заявки: canonical `POST /api/orders` повторяет activity lock/validation. Если товар успел стать inactive, заказ не создаётся, resolve не вызывается, заявка остаётся `PENDING` без `applied_order_id`.
