...
- При увеличении количества существующие `FILLED`-паллеты и валидные HU-reservations сохраняются; `PLANNED`/`PRINTED` паллеты строки не сбрасываются автоматически. После сохранения строки сервер append-only добавляет только `missing_qty = qty_ordered - filled_qty - planned_qty` для заказа в статусе `IN_PROGRESS`/`Draft`; `POST /api/orders/{orderId}/production-pallets/plan` использует ту же формулу и не выдает ошибку переназначения при наличии `FILLED`.
