...
- Pending-заявки `CREATE_ORDER` резервируют свой `order_ref`: `/api/orders/next-ref` учитывает еще не подтвержденные заявки, а PC web показывает такие строки со статусом `Ожидает подтверждения`.
