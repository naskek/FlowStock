...
- При уменьшении количества для `INTERNAL` защищенный минимум = `shipped_qty + filled_production_pallet_qty`; `PLANNED`/`PRINTED` без `FILLED` не считаются фактическим выпуском и не блокируют уменьшение. Для `CUSTOMER` защищенный минимум включает отгрузку и реальные `FILLED` production pallets по строке, но не включает складские `reserved_hu` из `order_receipt_plan_lines`: такой резерв является управляемой привязкой, а не фактом отгрузки. Нарушение блокируется сообщением вида `Нельзя уменьшить количество ниже уже заполненного/выпущенного объема: заполнено {qty}.`, с перечислением только реальных блокеров (отгрузка, FILLED production pallets).
- При уменьшении `CUSTOMER` сервер до валидации нормализует количество по текущим складским HU-резервам строки: выбирает детерминированный набор `order_receipt_plan_lines` в порядке `sort_order`, `id`, `to_hu` с максимальной суммой `qty_planned <= запрошенному qty`, оставляет только эти строки резерва и сохраняет `qty_ordered` равным их сумме. Если ни один HU не помещается в запрошенное положительное количество, резерв строки снимается, а запрошенное положительное qty остается как потребность к производству. HU неделимы; например `2400 -> 2300` при `4 x 600` сохраняет `1800` и 3 HU. Запрошенное `qty <= 0` для строки `CUSTOMER` отклоняется сообщением `Количество строки не может быть 0. Удалите строку заказа.`.
...
- `CUSTOMER`
  ...
  - TSD при выборе заказа загружает ожидаемые HU через `/api/tsd/outbound/orders/{orderId}`; для mixed pallet ответ содержит `hus[].lines[]` с составом HU (`item_id`, `order_line_id`, `item_name`, `qty`), а TSD показывает компоненты под HU-кодом. Scan HU вызывает `/api/tsd/outbound/orders/{orderId}/scan`, создает/использует draft `OUTBOUND` и добавляет строки по HU без записи в `ledger`. Строка scan берет `order_line_id`, `item_id`, `from_hu`, `from_location_id` и `qty` из merged-источника (plan-bound + `FILLED production_pallets`, qty = min(текущий HU stock, shipment remaining)). Повторный scan той же HU идемпотентен, включая уже auto-closed `OUTBOUND`, и не дублирует `doc_lines`/`ledger`
  - При `FlowStock:OutboundAutoCloseOnComplete = true` (default) TSD `complete` (и happy-path последний scan) закрывает `OUTBOUND`, пишет `ledger` и обновляет статус заказа без обязательного WPF close
...
- Диагностика производственного плана:
  ...
  - `problem_code`: `ORDER_ZERO_BUT_PALLETS_EXIST`, `PALLETS_EXCEED_ORDER_QTY`, `PRD_LINES_EXCEED_ORDER_QTY`, `FILLED_PALLETS_WITH_DRAFT_PRD`, `FILLED_PALLET_MISSING_LEDGER`, `SHIPPED_CUSTOMER_WITH_OPEN_PRD`, `MERGED_ORDER_WITH_PALLET_PLAN`, `CLOSED_PRD_LEDGER_MISMATCH`;
...
- удаление строки `CUSTOMER`-заказа через update/save удаляет связанный активный `PLANNED` pallet plan до удаления `order_lines`; `PRINTED`/`FILLED` паллеты блокируют удаление строки и не допускают появления активных orphan `production_pallets.order_line_id = NULL`;
