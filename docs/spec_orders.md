...
- В разделе `Маркировка` есть отдельная ручная кнопка `Создать маркировку`, которая вызывает `POST /api/marking/create-from-production-needs` и создает задачи маркировки по актуальной производственной потребности без дублирования уже покрытого объема.
  ...
  - Сервер заново пересчитывает актуальный `required_marking_qty` в момент POST и не принимает production qty от WPF/Web.
  ...
  - `marking_required_qty` описывает фактический remaining production output к маркировке: server-side production need и open `INTERNAL` remaining не суммируются безусловно, если это два представления одной и той же потребности.
  ...
  - Если свободные synthetic/production-based `marking_code` уже полностью покрывают текущий `required_marking_qty`, новая задача не создается.
  - Уже привязанные (`bound`) `marking_code` не гасят новую будущую production-based потребность; они используются только для close validation и автопривязки к `PRODUCTION_RECEIPT`.
