...
- При создании плана по заказу сервер дробит обычные строки по `items.max_qty_per_hu`, а строки с одинаковым `production_pallet_group` планирует как mixed pallet: один `production_pallets.hu_code` и несколько `production_pallet_lines`. Если capacity не задана, план не создается и возвращается ошибка `Не задано количество на паллете для номенклатуры`; для mixed pallet без правила вместимости возвращается `Не задано правило вместимости для микс-паллеты`. Последняя одиночная паллета может быть неполной.
...
- Открытый PRD не влияет на склад до fill-события. Успешный fill конкретной паллеты пишет `ledger` по всем строкам состава `production_pallet_lines`, переводит паллету в `FILLED` и учитывается в `receipt_remaining`; повторный fill возвращает `already_filled` и не дублирует `ledger`; закрытие такого PRD не дублирует `ledger`.
...
- Отдельный отчет `Потребность производства` считается независимо от контроля минимума и не заменяет его.
  ...
  - `raw_to_close_orders_qty` считается как оставшаяся потребность именно к производству, а не к отгрузке.
  - Для каждой строки `CUSTOMER`-заказа:
    - `customer_production_need_qty = max(0, qty_ordered - produced_or_reserved_coverage_qty)`.
...
- В WPF карточке заказа есть команды: `Сформировать план паллет`, `Удалить план паллет`, `Перенести план паллет`, `Печать паллетных этикеток`.
...
- `Перенести план паллет` является отдельным ручным действием для CUSTOMER-заказа и вызывает `POST /api/orders/{targetCustomerOrderId}/production-pallets/adopt-from-internal/{sourceInternalOrderId}`. Обычное сохранение CUSTOMER и auto-redistribute не выполняют этот перенос автоматически.
...
- Для INTERNAL-заказа, у которого вся потребность перенесена в CUSTOMER и выпуск больше не требуется, используется статус `MERGED` / display `Объединён`. Статус terminal: не участвует в auto-redistribute, TSD filling, production need и формировании плана паллет. Перевод в `MERGED` выполняется после успешного redistribute/adopt, если `qty_ordered = 0`, нет production/ledger/closed PRD и нет active pallet plan. При остаточном active pallet plan возвращается `SOURCE_INTERNAL_HAS_ACTIVE_PALLET_PLAN`.
...
- TSD `Наполнение` работает только с подготовленными известными HU: unknown HU отклоняется, HU другого заказа отклоняется, mixed pallet показывает состав `lines`, а `FILLED` паллета не пишет второй `ledger`.
- Закрытие palletized PRD не требует legacy-действия `Распределить по HU`; сервер проверяет активные `production_pallets` и блокирует только ненаполненные паллеты. Draft PRD с уже начатым `Наполнением` подсвечивается в списке документов.
...
- Смысл `Общий HU` перенесен из PRD в заказ/план паллет. В WPF карточке заказа у каждой строки есть галочка `Общий HU`; отмеченные строки получают группу паллеты, например `MIX-1`.
...
- После печати этикеток (`PRINTED`) или наполнения (`FILLED`) переназначение HU запрещено. Legacy PRD-флаг `Общий HU` не используется в palletized flow. Для PRD с активными `production_pallets` старые действия `Общий HU`, `Распределить по HU`, `Назначить HU` скрываются или disabled; legacy PRD без `production_pallets` сохраняет старое поведение.
...
- `CUSTOMER`
  ...
  - в TSD picker отгрузки попадают только клиентские `ACCEPTED`/`IN_PROGRESS` заказы с `has_shipment_remaining = true`
  - TSD при выборе заказа загружает строки через `/api/orders/{orderId}/shipment-remaining`, сохраняет `order_id/order_ref` в документе и `order_line_id` в строках
...
## Ручное перераспределение INTERNAL → CUSTOMER

- Назначение: перенести потребность с открытого внутреннего заказа на клиентский, чтобы не планировать лишний выпуск, пока товар еще не поступил на склад, и сразу привязать уже выпущенный объем к клиентскому заказу.
- Endpoint: `POST /api/orders/redistribute`
- Тело запроса:
  - `source_internal_order_id` — внутренний заказ-источник
  - `target_customer_order_id` — клиентский заказ-получатель
  - `item_id`
  - `qty` — объем переноса (> 0)
- Оба заказа должны быть не в финальных статусах (`SHIPPED`, `CANCELLED`).
- Перенос выполняется только между разными заказами; позиция должна существовать во внутреннем заказе.
- Сервер уменьшает `qty_ordered` строки внутреннего заказа и увеличивает `qty_ordered` соответствующей строки клиентского заказа (или создает строку, если ее не было).
- Объем переноса ограничен текущим `qty_ordered` внутренней строки; после переноса `qty_ordered` внутренней строки не может стать меньше уже выпущенного объема.
- Перенос делится на две части:
  - **невыпущенная часть** — уменьшает потребность внутреннего заказа и увеличивает клиентский `qty_ordered`;
  - **выпущенная часть** — допустима только если соответствующий объем уже есть на складе по HU, связанным с внутренним заказом (закрытый `PRD`, `FILLED production_pallets`, `origin/internal order` в read-model HU). Для этой части у клиентского заказа должен быть включен `bind_reserved_stock`, а тип номенклатуры — `enable_order_reservation = true`.
- При переносе сервер **сохраняет номера HU**: строки `order_receipt_plan_lines` внутреннего заказа снимаются с источника и добавляются клиентскому заказу с теми же `to_hu` (без переназначения на новые HU, если паллета уже напечатана).
- Для открытых `production_pallets` со статусом `PLANNED`/`PRINTED` по перенесенным HU сервер перепривязывает `order_id` / `order_line_id` к клиентскому заказу (этикетка на паллете не меняется).
- Оставшийся план внутреннего заказа подрезается по объему с сохранением HU (лишнее снимается с начала списка плана строки); полная пересборка вызывается только если плана не хватает до нового `qty_ordered`.
- Резервы остальных клиентских заказов с `bind_reserved_stock = true` пересчитываются FIFO; план получателя после переноса не затирается.
- Операция не изменяет `ledger`, закрытые документы и историю выпуска; меняются только `order_lines` и read-model резервов.
- Ответ содержит `qty_transferred`, `qty_from_unproduced`, `qty_from_produced_stock`, `source_qty_ordered_after`, `target_qty_ordered_after`.

## Автоперенос при сохранении клиентского заказа (WPF)

- После успешного создания/обновления клиентского заказа через сервер WPF вызывает `POST /api/orders/{target_customer_order_id}/auto-redistribute-from-internal`.
- Автоперенос выполняется только если у клиентского заказа включен `bind_reserved_stock` (пользователь подтвердил резерв складских HU).
- Сервер просматривает открытые `INTERNAL`-заказы (FIFO по `created_at`) и для каждой позиции клиентского заказа переносит совпадающий `item_id` через ту же логику, что и ручной `POST /api/orders/redistribute`.
- Объем одного переноса ограничен `min(qty_ordered клиентской строки, qty_ordered внутренней строки)`; перед переносом клиентская строка временно уменьшается на этот объем, чтобы итоговое `qty_ordered` клиента не росло (меняется только привязка выпуска/HU).
- Неудачные попытки (например, недостаточно выпущенного на складе) пропускаются без отмены сохранения заказа; в ответе API поле `ignored_attempts` содержит причины.
- При наличии переносов WPF показывает информационное окно со списком: внутренний заказ, товар, количество, HU, разбивка «из выпуска» / «со склада».

### Что видит оператор в WPF после save CUSTOMER

После успешного сохранения клиентского заказа WPF вызывает `POST /api/orders/{id}/auto-redistribute-from-internal` и **всегда** показывает окно «Результат сохранения заказа» (при ошибке сети/API — предупреждение, заказ при этом уже сохранён).

Ответ сервера (без изменения core-логики резерва) содержит:
- `bind_reserved_stock` — выполнялся ли сценарий резерва;
- `reservation_lines[]` — фактический план закреплённых HU (`hu_code`, `qty_planned`, `item_id`);
- `transfers[]` — успешные автопереносы с INTERNAL;
- `ignored_attempts[]` — пропущенные попытки с `reason_code` и `reason`;
- `warnings[]` — коды вроде `NO_HU_RESERVED`, `AUTO_TRANSFER_PARTIAL`, `CUSTOMER_RESERVATION_DISABLED`.

Оператор видит отдельные блоки: закреплённые HU, переносы, пропуски, предупреждения. Тихий успех без обратной связи не допускается.
...
- В paged-режиме `/api/orders` сортирует заказы по операторскому приоритету: `IN_PROGRESS`, затем `ACCEPTED`, затем `DRAFT`, затем `SHIPPED`, затем `CANCELLED`; тип `CUSTOMER`/`INTERNAL` не влияет на этот приоритет и не участвует в сортировке. Внутри одного статуса используется только `order_ref` по убыванию, без дополнительных server-side ключей сортировки. Старый вызов без `limit`/`offset` сохраняет прежнюю форму ответа.
