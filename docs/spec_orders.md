...
- Каждый новый INTERNAL → CUSTOMER production-plan adoption атомарно сохраняет durable provenance в `order_coverage_transfers` + `order_coverage_transfer_lines` до изменения ownership. Header фиксирует source/target order и PRD refs, pallet/HU, pallet status/printed timestamp на момент переноса и total transferred qty; lines фиксируют source/target order line IDs, item, исходный `doc_line`/component identity, transferred qty и snapshots `source qty_ordered / production_purpose / production_pallet_group`.
- Provenance является historical business record, а не operational ownership: IDs source/target orders/lines, PRD и pallet сохраняются как snapshots без FK на эти mutable/удаляемые сущности. Поэтому existing cleanup может удалить depleted source `order_line` или empty PRD, но история переноса остаётся читаемой; единственный каскадный FK — `order_coverage_transfer_lines.transfer_id → order_coverage_transfers.id`.
- Все server-facing INTERNAL → CUSTOMER production-plan adoption paths используют один authoritative mutation workflow: `AdoptSelectedProductionPallets` сохраняет provenance и меняет ownership, затем в той же transaction уменьшается source INTERNAL demand, выполняются safe cleanup source PRD/lines и `TryMarkAsMerged`. Legacy `AdoptPlanFromInternal` является compatibility wrapper: он строит ту же projection для указанного source order, требует чтобы весь выбранный source plan был adoptable в актуальную CUSTOMER shortage, переиспользует существующий открытый CUSTOMER PRD при наличии и не вызывает отдельную legacy mutation semantics.
- Миграция provenance additive и не выполняет heuristic backfill historical adoption по HU, comment или текущему ownership. Отсутствие provenance у старой записи не разрешает автоматически выдумывать source; legacy diagnostics/repair проектируются отдельно.
- Ошибка provenance insert или последующей adoption mutation откатывает и history, и ownership/demand changes одной PostgreSQL transaction. Прямой вызов datastore adoption вне внешней transaction также оборачивается внутренней atomic transaction.
- Uncompensated adopted `PLANNED`/`PRINTED` остаётся plan-layer и при отмене CUSTOMER либо при decrease/delete затронутой CUSTOMER line возвращается в source INTERNAL через provenance. Compensation всегда whole-transfer/whole-HU: mixed HU не дробится; если edit затронул хотя бы одну target component line, возвращается вся adopted HU, а оставшаяся CUSTOMER shortage дальше обслуживается обычным plan-flow.
- `RETURN_PLAN` compensation атомарно возвращает ownership `production_pallets`/`production_pallet_lines`/`doc_lines` в source INTERNAL, восстанавливает `qty_ordered` source lines ровно на transferred component qty и помечает `order_coverage_transfers.compensation_kind = 'RETURN_PLAN'` + `compensated_at`. Повторный вызов не увеличивает demand второй раз.
- Если source INTERNAL line была безопасно удалена после depletion, она восстанавливается из provenance snapshots (`item`, `production_purpose`, `production_pallet_group`) либо переиспользуется только однозначная эквивалентная текущая line. Если historical source PRD был удалён или уже `CLOSED`, closed history не переписывается: создаётся новый open `PRODUCTION_RECEIPT`, куда возвращается та же pallet/HU.
- Source INTERNAL в `MERGED` при `RETURN_PLAN` реактивируется из persisted source-status snapshot (`DRAFT`/`IN_PROGRESS`), после чего проходит canonical status refresh. `CANCELLED`/`SHIPPED` source не реактивируется автоматически.
- Для reversed `PRINTED` HU code сохраняется, но текущий print-state инвалидируется: operational pallet становится `PLANNED`, `printed_at = NULL`, поэтому этикетка должна быть напечатана заново для восстановленного INTERNAL context. Исторический `PRINTED` + `printed_at_at_transfer` остаётся в provenance.
- `RETURN_PLAN` запрещён, если adopted pallet уже `FILLED`, имеет component progress, ledger по этой HU/PRD, находится в закрытом target PRD либо operational composition не совпадает с provenance. При отмене CUSTOMER такой uncompensated transfer передаётся в factual compensation; decrease/delete CUSTOMER line по-прежнему fail-closed и не переписывает факт.
- Если adopted HU уже стала подтверждённым физическим stock (`FILLED` pallet + `CLOSED` target PRD + receipt ledger) и полный transferred composition этой HU всё ещё присутствует в текущем ledger stock, CUSTOMER cancellation применяет `RETURN_PRODUCED`. Closed PRD, historical `production_pallets`/`production_pallet_lines`/`doc_lines` и ledger не меняются и не переassign-ятся обратно в INTERNAL.
- `RETURN_PRODUCED` восстанавливает source INTERNAL demand на transferred component qty и сохраняет на каждой provenance line `compensated_source_order_line_id`: это current INTERNAL line, восстановленная/переиспользованная по тем же semantic snapshots, что `RETURN_PLAN`. Это поле не является ownership или FK на physical history.
- Transferred qty у `RETURN_PRODUCED` считается confirmed production credit исходного INTERNAL. `OrderReceiptRemainingCalculator`, canonical INTERNAL status и PostgreSQL production-need projection включают этот credit вместе с фактическим INTERNAL receipt, поэтому кейс `INTERNAL 300 → adopt 100 → fill → CUSTOMER cancel` даёт demand 300, confirmed 100 и remaining production 200 без повторного выпуска этих 100.
- CUSTOMER reservation освобождается обычным cancellation cleanup через `order_receipt_plan_lines`. После статуса target `CANCELLED` сохранённая produced HU больше не является CUSTOMER reserve/awaiting-shipment и остаётся обычным физическим warehouse stock, определяемым только ledger.
- `RETURN_PRODUCED` fail-closed, если target PRD не закрыт, pallet не является целиком `FILLED`, provenance/current composition расходятся либо transferred physical stock уже уменьшен/отгружен. Shipment/return semantics не имитируются искусственными ledger `-/+` и относятся к отдельному lifecycle workflow.
- Locks для cancel/edit строятся по target order и source INTERNAL orders из uncompensated provenance в стабильном порядке. После locks provenance перечитывается; если обнаружился новый source transfer, операция fail-closed и требует retry, вместо mutation без source lock.
...
- `Удалить план паллет` сначала читает `GET /api/orders/{orderId}/production-pallets/cancel-plan-options` и открывает выбор паллет по строкам заказа/номенклатуре. Доступны только `PLANNED`/`PRINTED` паллеты в DRAFT PRD без pallet/component fill progress и без ledger по тому же `PRD + HU + parent/component item`; unsafe/terminal строки видны disabled с конкретной причиной. Подтверждение вызывает `POST /api/orders/{orderId}/production-pallets/cancel-plan` с непустым списком выбранных `pallet_ids`.
...
- Writer не доверяет ранее полученным options: внутри transaction выполняет `LockOrdersForUpdate`, заново читает order/docs/pallet/component/ledger facts, повторяет eligibility и возвращает отсутствующие/ставшие unsafe IDs в `skipped_pallet_ids`. Финальный delete повторяет те же DRAFT/status/fill/ledger guards fail-closed.
- `CUSTOMER/SHIPPED`, `CANCELLED` и `MERGED` защищены от удаления плана. `INTERNAL/SHIPPED` допускает только safe cleanup future plan; WPF включает для него `Удалить план паллет` при наличии open plan, но оставляет `Сформировать план паллет` disabled.
- Выборочное удаление плана не меняет `FILLED` паллеты, закрытые PRD и любые строки `ledger`. Если по `PRINTED` паллете уже могла быть сформирована ЧЗ/маркировка, WPF показывает предупреждение и требует дополнительное подтверждение.
...
- Перенос разрешен только из INTERNAL-заказа в CUSTOMER-заказ: source должен быть `DRAFT`/`IN_PROGRESS`, target не должен быть `CANCELLED`/`SHIPPED`/`MERGED`; выбранный source PRD должен быть открытым `PRODUCTION_RECEIPT` с активными `PLANNED`/`PRINTED` паллетами, без `ledger`, `FILLED` и partial progress. Наличие существующего открытого CUSTOMER production plan не является blocker: legacy compatibility endpoint переиспользует актуальный открытый target PRD и применяет тот же authoritative adoption workflow.
- При переносе сервер в одной транзакции сохраняет provenance, переносит `doc_lines`, `production_pallets` и `production_pallet_lines` на target PRD, ремапит `order_line_id` через ту же projection, что selected/pre-plan flow, сохраняет HU/status/qty/location/timestamps и не меняет `ledger`, закрытые документы или OUTBOUND. Затем `qty_ordered` source INTERNAL уменьшается ровно на transferred component qty; depleted source lines/PRD очищаются безопасно и `TryMarkAsMerged` применяется по тем же правилам, что в selected flow. Если в target нет подходящей строки заказа для любого `item_id` выбранного source plan, сервер возвращает `TARGET_LINE_NOT_FOUND`/`SOURCE_PLAN_NOT_ADOPTABLE` без частичного переноса.
...
- После изменения количества INTERNAL-заказа pallet-plan sync/trim и `OrderService.RefreshPersistedStatus` выполняются в одной update transaction; persisted `SHIPPED`/`IN_PROGRESS` не зависит от последующего read API.
- Для `CUSTOMER`:
...
- `problem_code`: `ORDER_ZERO_BUT_PALLETS_EXIST`, `PALLETS_EXCEED_ORDER_QTY`, `PRD_LINES_EXCEED_ORDER_QTY`, `FILLED_PALLETS_WITH_DRAFT_PRD`, `FILLED_PALLET_MISSING_LEDGER`, `PARTIAL_PALLET_HAS_LEDGER`, `PARTIAL_PALLET_INVALID_STATUS`, `SHIPPED_CUSTOMER_WITH_OPEN_PRD`, `MERGED_ORDER_WITH_PALLET_PLAN`, `CLOSED_PRD_LEDGER_MISMATCH`;
  - `pallet_filled_qty` сохраняет публичную семантику total aggregate. Open-FILLED anomalies используют отдельный внутренний aggregate только по open DRAFT PRD: закрытый `FILLED` не создаёт `FILLED_PALLET_MISSING_LEDGER` для другого открытого PRD, а open `FILLED` без положительного ledger по тому же `PRD + item + HU` создаёт;
  - partial component progress при persisted `PLANNED`/`PRINTED` является historical anomaly даже при отсутствии ledger: canonical presentation — `INCONSISTENT`, normal fill его не продолжает;
  - для `INTERNAL` `PALLETS_EXCEED_ORDER_QTY` сравнивает только active future qty (`PLANNED`/`PRINTED` в open DRAFT PRD) с `max(0, order_qty - confirmed PRD ledger qty)`. Проверка выполняется после более критичных closed-ledger/open-FILLED/partial inconsistencies; `CUSTOMER/SHIPPED` из exceed-проверок исключён;
