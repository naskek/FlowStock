...
- Additive read model не используется как permission. Отдельная server-owned command policy теперь fail-closed проверяет shipment lifecycle: раннее offer/candidate filtering, повторную validation после locks на reservation/binding/scan/autofill и обязательный authoritative `OUTBOUND Close`. PostgreSQL mutation перечитывает HU facts через scoped store той же connection/transaction после канонического порядка order → normalized HU → document locks. `INCONSISTENT`, mixed в item-scoped binding, foreign reservation/DRAFT и stale composition не допускаются; текущий OUTBOUND document не конфликтует сам с собой. Outbound TSD сохраняет отдельный command/eligibility read-model.
