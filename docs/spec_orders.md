...
`marking_order` для производственной потребности может иметь `order_id = NULL`. Связь с источником задается через `source_type`: `PRODUCTION_NEED` для агрегированной производственной потребности и `PRODUCTION_ORDER` для будущей привязки к конкретному производственному заказу/draft. Кнопка `Создать маркировку` не ищет клиентские заказы напрямую, а использует server-side расчет производственной потребности и открытого `INTERNAL` remaining.
...
В WPF и PC Web рядом с заказом используется только бинарная подсказка ЧЗ: `Маркировка проведена` или `Маркировка не проведена`. Заказ без маркируемых товаров может не показывать индикатор.
