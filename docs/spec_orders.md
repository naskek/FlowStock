...
- При создании плана по заказу сервер дробит строки по `items.max_qty_per_hu`, генерирует HU в формате `HU-0000001` через server-side sequence и не пишет `ledger`. Если capacity не задана, план не создается и возвращается ошибка `Не задано количество на паллете для номенклатуры`; последняя паллета может быть неполной.
...
- Открытый PRD не влияет на склад до fill-события. Успешный fill конкретной паллеты пишет `ledger`, переводит паллету в `FILLED` и учитывается в `receipt_remaining`; закрытие такого PRD не дублирует `ledger`.
...
- `docs(order_id)`
- `production_pallets(prd_doc_id, doc_line_id)` уникальный
...
- BarTender `.btw` шаблон получает NamedSubStrings: `HuCode`, `ItemName`, `Qty`, `OrderRef`, `PrdRef`, `Brand`, `Uom`, `PalletNo`, `PalletCount`, `StoragePlace`, `ProductionDate`, `Comment`.
...
- TSD `Наполнение` работает только с подготовленными известными HU: unknown HU отклоняется, а `FILLED` паллета не пишет второй `ledger`.
...
Для `INTERNAL`:
- `produced_qty` = сумма legacy `doc_lines.qty` по закрытым PRD без `production_pallets` + сумма `production_pallets.planned_qty` по `status = FILLED`; для закрытого palletized PRD эти источники не суммируются повторно
