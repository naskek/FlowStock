...
- Scoped-синхронизация плана после сохранения WPF/API всегда расширяет строку с `production_pallet_group` до всех строк того же `order_id + production_pallet_group`, чтобы группа создавалась атомарно одним mixed HU, даже если причиной синхронизации было изменение одной строки.
- Для `INTERNAL`-заказа повторное планирование идемпотентно и добавляет только нехватку по строке: `max(0, qty_ordered - active_production_pallet_qty)`. `active_production_pallet_qty` считается по всем PRD заказа, включая `FILLED` паллеты, перенесенные в dedicated PRD через TSD auto-close; статусы `PLANNED`, `PRINTED`, `FILLED` считаются покрытием, `CANCELLED` не считается. Если у паллеты есть `production_pallet_lines`, покрытие строки считается только по `production_pallet_lines.order_line_id/planned_qty`, без fallback на `production_pallets.order_line_id/planned_qty`.
- При увеличении количества строки уже palletized `INTERNAL`-заказа сервер сохраняет существующие плановые HU и добавляет только дельту паллет с новыми уникальными production HU через server sequence; свободные складские HU/ledger stock не используются для будущего производства.
- Для `CUSTOMER`-заказа план паллет на выпуск создается только на нехватку по строке: `max(0, qty_ordered - qty_shipped - customer_bound_hu_qty - active_production_pallet_qty)`.
...
- При создании/обновлении `INTERNAL`-заказа сервер дробит строки по `max_qty_per_hu` и резервирует серверные HU в `order_receipt_plan_lines`. Для `CUSTOMER` складской HU-резерв меняется только явным apply-flow, а обычное сохранение заказа не запускает FIFO и не пересобирает `order_receipt_plan_lines`.
