...
- Отдельный отчет `Потребность производства` считается независимо от контроля минимума и не заменяет его.
  ...
  - `existing_marking_qty` берется только из `marking_order` с `source_type = PRODUCTION_NEED` или `PRODUCTION_ORDER`; старые/использованные задачи без production-source не гасят новую производственную потребность.
