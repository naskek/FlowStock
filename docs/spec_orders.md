...
- Основной workflow ЧЗ запускается из карточки заказа через `POST /api/orders/{orderId}/marking/export`; источником расчета являются серверные `orders` и `order_lines`, а не очередь производственной потребности.
  ...
  - Web-список заказов показывает бинарный статус ЧЗ из server-side DTO: `Маркировка не проведена` / `Маркировка проведена`.
...
- В списке заказов PC web есть компактный индикатор `ЧЗ`:
  - красный `Маркировка не проведена`, если effective-статус `REQUIRED`;
  - зеленый `Маркировка проведена`, если effective-статус `PRINTED`.
