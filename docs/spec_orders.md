...
- Partial progress mixed HU хранится только в `production_pallet_lines.filled_qty` и `production_pallet_lines.filled_at`. `PARTIALLY_FILLED` является derived effective status, не сохраняется в `production_pallets.status` и не считается finished goods stock.
- Существующие заказы и legacy PRD без `production_pallets` остаются legacy: они не появляются в TSD `Наполнение` и не конвертируются автоматически. План паллет создаётся только явными серверными действиями, и таких пути два: основной — конструктор паллет (`GET .../production-pallets/plan-preview` + `POST .../production-pallets/plan-explicit`, включая mixed), и legacy single-item `POST /api/orders/{orderId}/production-pallets/plan` (см. раздел «Конструктор паллет»).
- Легаси-endpoint `POST /api/orders/{orderId}/production-pallets/plan` создаёт только **single-item** план: дробит производственную нехватку каждой строки по `items.max_qty_per_hu` (последняя паллета может быть неполной); если capacity не задана — ошибка `Не задано количество на паллете для номенклатуры`. Если среди планируемых строк есть непустой `production_pallet_group`, endpoint возвращает `LEGACY_MIXED_GROUP_NOT_SUPPORTED` и не создаёт PRD/HU. Mixed-паллеты формируются только конструктором (`plan-explicit`, см. раздел «Конструктор паллет»), где `items.max_qty_per_hu` обязателен и проверяется (equal-cap).
...
- Mixed component flow доступен только при `ProductionAutoCloseOnFill=true`: `POST /api/tsd/production/fill-mixed-pallet-components` сохраняет выбранные строки идемпотентно, а последний компонент в той же транзакции переводит паллету в `FILLED` и запускает dedicated PRD Close. Ошибка Close откатывает только текущий запрос, не затрагивая ранее сохранённый partial progress. Старый `fill-pallet` возвращает `MIXED_COMPONENT_SELECTION_REQUIRED` для незавершённой mixed HU и `already_filled` для финализированной.
- Partial mixed HU сохраняет persisted `PLANNED`/`PRINTED`, не участвует в OUT, ready-HU binding, produced stock, CUSTOMER reserve и release-produced-stock; паллету с component progress нельзя удалить, отменить, trim/replan/adopt или переназначить. Ручной Close mixed PRD запрещён, пока заполнены не все компоненты.
- Partial mixed HU имеет derived `PARTIALLY_FILLED`, остаётся fillable (`can_fill=true`) и не удаляет заказ из TSD `Наполнение`. Повторный scan того же HU возвращает завершённые component lines как `is_completed=true` и позволяет выбрать оставшиеся; только последний компонент переводит HU в `FILLED`.
...
## WPF: HU по строке заказа

- Действие/кнопка **`HU по строке`** не отключается только из-за того, что по строке уже есть production pallet plan (mixed coverage).
- Оператор должен иметь возможность **просматривать и редактировать** HU-привязку строки при наличии и склаского reserve, и planned production.
- Колонка **`HU по строке`** в гриде строк заказа должна быть **достаточно широкой**; длинное наименование товара можно сжимать сильнее, чем колонку HU.
- `OrderLineView.HuDisplayRows` является объединённым display-only представлением: активные `PLANNED`/`PRINTED` строки будущего выпуска сохраняются и дополняются судьбой `FILLED` HU. Fate-строки не меняют operational `production_hu_codes`, HU-кандидаты, резерв, ledger, документы или производственный план.
- В source-заказе для `FILLED` HU показывается положительный текущий остаток (`наполнено`), активный CUSTOMER-резерв (`→ резерв заказ N`) либо наиболее поздняя проведённая CUSTOMER-отгрузка (`→ отгружено заказ N`). Проведённая отгрузка имеет приоритет над резервом и остаётся видимой при нулевом текущем остатке.
- В target CUSTOMER-заказе резерв и проведённая отгрузка показывают достоверно найденный source-заказ (`← выпуск заказ N`). Если source установить нельзя, стрелка не выводится; legacy `CLOSED PRODUCTION_RECEIPT` без нормализованного HU и ledger по тому же `doc + item + HU` не создаёт fate-строку.
- Для mixed production pallet source определяется по `production_pallet_lines`. Если source и target совпадают, лишняя стрелка подавляется. Порядок строк детерминирован: `PLANNED`/`PRINTED`, затем `FILLED` с положительным остатком, резерв, историческая отгрузка; внутри категории — по коду HU.
...
- `Создать/Дополнить/Открыть план паллет` открывает серверно-авторитетный конструктор (`plan-preview` + `plan-explicit`, см. «Конструктор паллет»): сервер строит нехватку и авто-разбиение, оператор задаёт физические паллеты, confirm атомарно создаёт `PRODUCTION_RECEIPT`/`doc_lines`/`production_pallets`, назначает HU через `hu_code_seq` и не пишет `ledger`. Легаси-endpoint `POST /api/orders/{orderId}/production-pallets/plan` остаётся только single-item и отклоняет строки с `production_pallet_group`.
...
- Mixed-паллеты формируются **только** серверно-авторитетным конструктором паллет (preview/confirm, см. раздел «Конструктор паллет»), а не колонками строки заказа. Колонки `Общий HU` и `Группа HU` из карточки заказа удалены; новый WPF не редактирует `production_pallet_group`.
- `production_pallet_group` — **инертное legacy-поле**: колонка БД и API-совместимость сохраняются, но поле не является источником состава паллеты и не задаётся из UI. Удаление поля и миграция данных — отдельная обоснованная фаза.
- Legacy-путь `POST /api/orders/{orderId}/production-pallets/plan` больше **не** формирует mixed-паллеты из группы: если среди планируемых строк есть непустой `production_pallet_group`, сервер возвращает структурную ошибку `LEGACY_MIXED_GROUP_NOT_SUPPORTED` (HTTP 400) и не создаёт PRD/HU. Без групп endpoint строит только single-item план с обязательным `items.max_qty_per_hu`. Санкционированный путь mixed — конструктор (`plan-explicit`).
- Вместимость mixed-паллеты (MVP): mixed разрешён только когда у всех компонентов задан одинаковый положительный `items.max_qty_per_hu`; вместимость = это значение; сумма компонентов ≤ вместимости; неполная паллета допустима. Разные лимиты сервер отклоняет ошибкой `PALLET_CAPACITY_MISMATCH`, превышение — `PALLET_OVER_CAPACITY`.
- В строках заказа WPF показывает `HU по строке` из operational `production_pallets`/`production_pallet_lines`: для каждого HU — количество данного компонента, общий объём HU и признак `МИКС` (формат `HU · <label> · <qty> из <total> · МИКС`). Один физический mixed HU отображается в каждой участвующей строке только со своим количеством.
...
- После наполнения (`FILLED`) переназначение/удаление HU запрещено. Наличие `PRINTED` или существующего `PLANNED` плана не блокирует append-only добавление недостающих `PLANNED` HU при увеличении `qty_ordered`; флаг `printed` сам по себе не запрещает дополнение плана. Для PRD с активными `production_pallets` старые действия `Распределить по HU` и `Назначить HU` скрываются или disabled; legacy PRD без `production_pallets` сохраняет старое поведение.

## Конструктор паллет (preview/confirm)

Оператор задаёт физические паллеты и количество каждого товара на каждой явным серверно-авторитетным конструктором. Клиент не рассчитывает нехватку и не принимает окончательное решение.

- `GET /api/orders/{orderId}/production-pallets/plan-preview` — read-only preview: текущая серверная производственная нехватка по строкам, гомогенное авто-разбиение (`suggested_pallets`, дробление по `items.max_qty_per_hu`, только на текущую нехватку), существующий read-only план (`open_plan_pallets` — активные `PLANNED`/`PRINTED`/`PARTIALLY_FILLED`), историю (`historical_pallets` — `FILLED`), `production_required` и `preview_fingerprint`. `suggested_pallets` возвращаются всегда (пусто при нулевой нехватке). Для CUSTOMER нехватка учитывает отгрузку, warehouse-bound HU и активные production pallets; для INTERNAL — confirmed coverage. Существующие паллеты read-only; удаляемость каждой отражена `can_delete`/`disabled_reason` (PLANNED/PRINTED — удаляемы через существующий selected-delete; PARTIALLY_FILLED — «частично наполнена»; FILLED — история; CLOSED PRD — «выпуск закрыт»).
- `POST /api/orders/{orderId}/production-pallets/plan-explicit` — **append-delta**. Тело: `preview_fingerprint` + список паллет с компонентами `{ order_line_id, qty }` (только временный план дельты; `item_id`/HU/PRD/статус сервер выводит сам). В одной транзакции под `LockOrdersForUpdate([orderId])` сервер перечитывает состояние, сверяет `preview_fingerprint` (иначе `PLAN_PREVIEW_STALE`), проверяет: заказ планируемый (`ORDER_NOT_PLANNABLE`); каждая `order_line_id` принадлежит заказу и не отменена (`ORDER_LINE_NOT_FOUND`/`ORDER_LINE_CANCELLED`); входной план не переписывается молча — пустая паллета либо нулевое/отрицательное/не-конечное количество компонента дают `INVALID_PALLET_PLAN` (HTTP 400) без каких-либо записей; equal-cap (`PALLET_CAPACITY_MISMATCH`/`PALLET_OVER_CAPACITY`); распределение по каждой строке точно равно нехватке (`LINE_ALLOCATION_MISMATCH` с `details.lines[{order_line_id, required_qty, allocated_qty, difference_qty}]`); нулевая нехватка → `NO_PRODUCTION_REQUIRED`. Только затем создаёт/переиспользует open DRAFT `PRODUCTION_RECEIPT`, генерирует HU через `hu_code_seq`, добавляет `doc_lines`/`production_pallets`/`production_pallet_lines` **только на дельту** и не пишет `ledger`. Существующие `PLANNED`/`PRINTED`/`PARTIALLY_FILLED`/`FILLED` паллеты, их `doc_lines`, PRD, progress и `ledger` не изменяются. Ошибка откатывает всё; разрывы `hu_code_seq` после отката допустимы.
- Повторное явное планирование — append только актуальной нехватки; существующие `PLANNED`/`PRINTED`/`FILLED` считаются coverage. Наличие открытого плана само по себе **не** блокирует добавление новой дельты. Перестройка существующего плана — отдельная операция: удалить допустимые `PLANNED`/`PRINTED` через существующий selected-delete, затем preview→confirm. `PARTIALLY_FILLED` перестраивать нельзя, но она не блокирует append независимой новой дельты.
- Concurrency: `plan-explicit` сериализуется строкой `orders` (тот же row lock id ASC, что Close/Outbound/OrderControl; обычный order update удерживает эту же строку через `UPDATE orders`). Два параллельных confirm одной дельты дают ровно один план: второй после лока видит изменённый fingerprint и возвращает `PLAN_PREVIEW_STALE`. Повторный submit не создаёт второй комплект HU. Новый fingerprint успешного confirm вычисляется внутри той же транзакции (после материализации плана, до освобождения лока) и возвращается в ответе; endpoint не выполняет повторное business-чтение после commit.
- Ранняя orders-lock семантика конкурирующих coverage/qty-writer'ов: все операторские пути, изменяющие CUSTOMER/INTERNAL coverage (`order_receipt_plan_lines`) или production-план, берут `orders` row lock (`LockOrdersForUpdate`) **до** бизнес-чтений заказа, строк, `order_receipt_plan_lines`, кандидатов/read-model и любых замен coverage/отмен паллет; все затрагиваемые source/target заказы блокируются одним набором (примитив сортирует id ASC), состояние перечитывается после лока. Полный перечень защищённых путей:
  - `plan-explicit` (`ProductionPalletService.ConfirmExplicitPlan`) — `LockOrdersForUpdate([orderId])`.
  - HU-binding `apply-final` (`OrderHuBindingApplyFinalService`) — блокирует затронутые заказы, затем заменяет coverage и отменяет surplus customer planned pallets.
  - Legacy explicit reservation `POST /api/orders/{orderId}/hu-reservations/apply` (`OrderHuReservationApplyService`) — `LockOrdersForUpdate([customerOrderId])` до `GetOrder`/`GetOrderLines`/candidate-чтений; после замены `order_receipt_plan_lines` отменяет ставший surplus future-plan (та же реконсиляция, что apply-final), поэтому reserved coverage + активная продукция не превышают потребность.
  - Bulk ready-HU binding (`OrderHuBindingManageApplyService`) — извлекает затронутые `order_id` из валидированной формы запроса и берёт единый `LockOrdersForUpdate` по всем ним **до** `LoadSnapshot`; snapshot перечитывается после лока.
  - Резерв готового HU `POST /api/orders/{orderId}/reserve-produced-hu` (`OrderProducedHuReservationService`) — `LockOrdersForUpdate([sourceInternalOrderId, targetCustomerOrderId])` до чтения source/target/строк/stock/context; повторный вход из уже заблокированного `OrderRedistributionService` в той же транзакции безопасен (Postgres re-lock — no-op). После замены coverage целевого заказа отменяет surplus future-plan той же реконсиляцией.
  - Ручное перераспределение (`OrderRedistributionService`) и автоперенос (`OrderAutoRedistributionService`) — блокируют весь набор source/target до чтений и записей qty/coverage.
  - Обычный order update (`OrderService.UpdateOrder`), OUTBOUND close (`DocumentService`), internal merge (`InternalOrderMergeService`) сериализуются той же строкой `orders` через `store.UpdateOrder`/`LockOrdersForUpdate`.
- Fingerprint остаётся stale-защитой после лока, а не заменой блокировке. TSD fill hot-path orders-lock не берёт (изменение PLANNED/PRINTED→FILLED нейтрально для нехватки); maintenance-only ремонтные/backfill endpoints (`NewLedgerTransitionService`, `OrderReservationBackfillService`) и downstream read-model resync (`RebuildInternal/CustomerOrderReceiptPlan`, `RefreshCustomerReceiptPlansCore`) вызываются из уже заблокированных путей и отдельного orders-lock не берут.
...
- `CUSTOMER`
  ...
  - в TSD picker отгрузки попадают клиентские заказы, готовые к подбору: `ACCEPTED` (`Готов`) либо еще `IN_PROGRESS`, но уже полностью покрытые ожидаемыми HU без производственной нехватки. Ожидаемые HU берутся как из `order_receipt_plan_lines` (customer-bound HU с положительным `ledger`), так и из `production_pallets` со статусом `FILLED` для этого заказа (паллетизированный выпуск) с положительным текущим HU/item stock и остатком к отгрузке по строке
  - TSD при выборе заказа загружает ожидаемые HU через `/api/tsd/outbound/orders/{orderId}`; для mixed pallet ответ содержит `hus[].lines[]` с составом HU (`item_id`, `order_line_id`, `item_name`, `qty`), а TSD показывает компоненты под HU-кодом. Scan HU вызывает `/api/tsd/outbound/orders/{orderId}/scan`, создает/использует draft `OUTBOUND` и добавляет строки по HU без записи в `ledger`. Строка scan берет `order_line_id`, `item_id`, `from_hu`, `from_location_id` и `qty` из merged-источника (plan-bound + `FILLED production_pallets`, qty = min(текущий HU stock, shipment remaining)). Повторный scan HU в текущем draft идемпотентен; HU из уже закрытого `OUTBOUND` отклоняется как уже отгруженный, а повторный `complete` закрытого TSD OUT не дублирует `ledger`
  ...
  - partial OUT разрешает выбрать не все HU заказа, но выбранная mixed HU отгружается только целиком: WPF удаляет из draft сразу все component-lines HU после подтверждения и запрещает редактировать количество отдельного компонента; Close неполной mixed HU возвращает `MIXED_HU_SHIP_AS_WHOLE_REQUIRED` до записи в `ledger`
- `INTERNAL`
...
- WPF order-scoped flow открывается из карточки сохраненного `CUSTOMER`-заказа кнопкой `Привязка HU`; для `INTERNAL` заказов кнопка скрыта. Если в карточке есть несохраненные изменения, оператор сначала сохраняет заказ, чтобы окно не работало поверх stale local state.
- Окно `Привязка HU к заказу` показывает слева свободные HU candidates только для текущего заказа, справа — будущую привязку HU по строкам этого заказа. Кнопки `>` и `<` выполняют staged bind/detach локально, `Авто` локально подбирает HU по FIFO-порядку candidates, а база меняется только по `Сохранить и закрыть`.
- Новый WPF order-scoped flow сохраняет итоговое состояние через `POST /api/orders/{orderId}/hu-bindings/apply-final`: `expected_bound_hu_codes` берется из saved snapshot при открытии окна, `final_hu_codes` — из правой панели. Старый `POST /api/orders/{orderId}/hu-reservations/apply` остается legacy/manual endpoint для старых явных действий и не используется новым окном.
...
- Контракт WPF/server для нового CUSTOMER HU binding: кандидаты читаются через `POST /api/orders/hu-reservation-candidates`, применение выполняется через `POST /api/orders/{orderId}/hu-bindings/apply-final`.
...
- Для `CUSTOMER`:
  ...
  - статус `ACCEPTED` (UI: `Готов`) выставляется только если по всем строкам с `qty_ordered > tolerance` canonical `missing_qty <= tolerance`: учитываются отгрузка, текущие bound ledger HU, подтвержденные filled production HU с положительным ledger и legacy confirmed non-HU выпуск без двойного счета HU.
...
- `problem_code`: `ORDER_ZERO_BUT_PALLETS_EXIST`, `PALLETS_EXCEED_ORDER_QTY`, `PRD_LINES_EXCEED_ORDER_QTY`, `FILLED_PALLETS_WITH_DRAFT_PRD`, `FILLED_PALLET_MISSING_LEDGER`, `PARTIAL_PALLET_HAS_LEDGER`, `PARTIAL_PALLET_INVALID_STATUS`, `SHIPPED_CUSTOMER_WITH_OPEN_PRD`, `MERGED_ORDER_WITH_PALLET_PLAN`, `CLOSED_PRD_LEDGER_MISMATCH`;
  - partial component progress допустим без ошибки только при persisted `PLANNED`/`PRINTED`, заполненной части строк и отсутствии ledger по HU; persisted `FILLED` без ledger по-прежнему возвращает `FILLED_PALLET_MISSING_LEDGER`;
...
- Для расчета готовности/паллетных индикаторов по загруженной странице PC web использует batch-read строк `GET /api/orders/lines?ids=1,2,3`, а не последовательные `GET /api/orders/{id}/lines` по каждому заказу. Ответ batch endpoint содержит элементы `{ order_id, lines[] }` в порядке запрошенных id; неизвестный `order_id` возвращается с пустым `lines`. Batch endpoint сохраняет быстрый совместимый контракт и не загружает подробную HU-историю. Одиночный `GET /api/orders/{id}/lines` для карточки заказа использует те же базовые поля и может дополнительно возвращать optional read-only поля `warehouse_hu_rows`, `production_hu_rows`, `shipped_hu_rows` и nullable `coverage`. Строки `production_hu_rows` могут содержать optional `fate_code`, `fate_label`, `fate_order_ref`, `fate_doc_ref`, `fate_qty`, полученные из того же display-only fate read-model, что используется WPF `HU по строке`.
...
- В строках заказа в WPF и PC web отображаются наименование, SKU/штрихкод и GTIN. Модальное окно деталей заказа PC web сохраняет основные колонки `Товар`, `SKU / ШК`, `GTIN`, `Заказано`, `Наполнение`; колонки `В наличии`, `Назначение`, `Отгружено`/`Выпущено` в основной строке не выводятся. Верхняя часть модального окна не дублирует статус заказа, паллет и маркировки отдельным рядом icon-only индикаторов; построчный индикатор `Наполнение` сохраняется в контексте конкретной строки. Каждая строка раскрывается read-only и показывает production pallets/HU, а для `CUSTOMER` секция `HU по строке заказа` объединяет текущие явно привязанные ledger-HU и уже отгруженные HU строки. Текущая складская привязка и проведённая отгрузка остаются разными состояниями; при частичной отгрузке один HU может отображаться отдельными строками с текущим и отгруженным количеством. Точный серверный итог покрытия показывается при его наличии. Для production HU отдельно показывается `Движение HU` из WPF-compatible fate read-model: текущий stock, активный резерв другого заказа или наиболее поздняя проведённая CUSTOMER-отгрузка; передача произведённого HU в другой CUSTOMER-заказ отображается как `Передано в заказ N`. Только для `CUSTOMER` секция `Отгрузка этой строки заказа` показывает OUTBOUND текущей строки и не смешивается с движением HU, произведённого исходным INTERNAL/CUSTOMER-заказом. Для `INTERNAL` CUSTOMER-секция `HU по строке заказа` и секция отгрузки не показываются, а серверные значения coverage отображаются как `Итог выпуска`, `Выпущено` и `Осталось выпустить`. Fate-данные display-only и не участвуют в coverage. Раскрытие не содержит команд редактирования/rebind и не вычисляет `covered_qty`/`missing_qty` на frontend; если точный `coverage` отсутствует, показываются HU-детали и существующий серверный `shortage`. Для `INTERNAL` клиентские формулировки HU binding скрыты. Таблица динамически занимает всю доступную ширину модального окна, распределяет ширину между колонками и на узком экране прокручивается внутри модального окна. Строковый прогресс паллетного наполнения берется из серверных `filled_pallet_count/planned_pallet_count` и `pallet_filled_qty/pallet_planned_qty`; полностью наполненные строки могут показывать только icon-only индикатор наполнения с подсказкой, частично наполненные строки сохраняют видимый текстовый прогресс. Зеленая подсветка строки `INTERNAL` включается только когда фактически произведенный/наполненный объем покрывает заказанное количество; частичное наполнение или один только `PLANNED`-план не подсвечиваются зеленым как готовые. Зеленая подсветка строки `CUSTOMER` включается только по canonical `coverage.missing_qty <= tolerance` или совместимым серверным `shortage/can_ship_now`; pallet fill progress не участвует в shipment readiness.
...
- WPF оператор обрабатывает заявки во вкладке `Требуют действия`:
...
- Computed `READY_HU_BINDING_AVAILABLE` показывается в WPF incoming requests как отдельный read-only тип `Готовые HU`, если computed `hu_count > 0`. Фильтр типа запроса не меняет загрузку persistent item/order requests; `ShowResolved` применяется только к ним. В Phase 4C details/double-click открывает `Глобальная привязка готовых HU`: оператор может только staged-добавлять свободные ready HU к compatible CUSTOMER lines, без detach уже сохраненных HU. `Авто` является локальным разовым greedy FIFO-подбором и не вызывает API. `Сохранить` вызывает только `/api/orders/{orderId}/hu-bindings/apply-final` последовательно по заказам; при partial failure успешные заказы остаются сохраненными, их staged entries удаляются, а failed/not-run entries остаются видимыми с инструкцией обновить список перед продолжением.
