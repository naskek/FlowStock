...
- В разделе `Маркировка` есть отдельная ручная кнопка `Создать маркировку`, которая вызывает `POST /api/marking/create-from-production-needs` и создает задачи маркировки по актуальной производственной потребности без дублирования уже покрытого объема.
  ...
  - Повторное создание маркировки идемпотентно: `marking_to_create = max(0, required_marking_qty - existing_marking_qty)`.
  - `existing_marking_qty` считается из свободных `marking_code` по item/GTIN и активной еще не обеспеченной части `marking_order` с `source_type = PRODUCTION_NEED` или `PRODUCTION_ORDER`.
  - Привязанные/использованные коды и старые задачи без production-source не гасят новую производственную потребность; новая `marking_order` создается, если свободных кодов не хватает.
...
Новая производственная потребность покрывается только свободными кодами (`receipt_line_id IS NULL`) и активными незавершенными задачами, у которых кодов еще меньше `requested_quantity`. Уже использованные/привязанные коды не гасят новую потребность; если свободных кодов не хватает, создается новая `marking_order`.
