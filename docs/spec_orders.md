...
- **Ожидаемый внутренний выпуск** — остаток к выпуску `max(0, qty_ordered - produced_qty)` по строкам открытых `INTERNAL`-заказов (`status NOT IN (SHIPPED, CANCELLED, MERGED)`; `DRAFT` и `IN_PROGRESS` считаются открытыми). Это не ledger-остаток и не резерв: он не уменьшает потребность, не блокирует планирование и не участвует в coverage.
- Перед явным планированием паллет `CUSTOMER`-заказа UI может показать server-derived warning через read-only `GET /api/orders/{orderId}/production-pallets/internal-supply-warning`: endpoint не меняет БД и возвращает `has_warning`, человекочитаемое `message` и строки `customer_order_line_id`, `item_id`, `item_name`, `would_plan_qty` (нехватка строки по той же формуле, что использует `POST /plan`), `internal_order_id`, `internal_order_ref`, `internal_status`, `expected_qty`. Warning срабатывает при любом пересечении: `would_plan_qty > 0` и положительный ожидаемый внутренний выпуск по тому же `item_id`. Для `INTERNAL`-заказа и terminal-статусов endpoint возвращает `has_warning = false`.
- WPF при `has_warning = true` показывает модальное подтверждение с кнопками `Всё равно сформировать` / `Отмена`; отмена не вызывает `POST /plan`. Если preview недоступен из-за ошибки сервера/сети (кроме HTTP 404 старого сервера без endpoint — тогда лог и продолжение), WPF показывает нейтральное подтверждение `Не удалось проверить ожидаемый INTERNAL-выпуск` с теми же кнопками. Warning не резервирует товар, не меняет `ledger` и формулу планирования; клиент не передает рассчитанные количества, окончательное создание паллет остаётся за серверным пересчётом в `POST /api/orders/{orderId}/production-pallets/plan`.
