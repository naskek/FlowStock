...
- TSD fill palletized PRD атомарен: `production_pallets.status = FILLED`, закрытие PRD и положительные строки `ledger` фиксируются в одной транзакции. Если close/ledger падает, статус паллеты откатывается, PRD остается в прежнем состоянии, `ledger` не получает частичных строк.
