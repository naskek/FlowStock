...
- `CUSTOMER`
  ...
  - TSD при выборе заказа загружает ожидаемые HU через `/api/tsd/outbound/orders/{orderId}`; scan HU вызывает `/api/tsd/outbound/orders/{orderId}/scan`, создает/использует draft `OUTBOUND` и добавляет строки по HU без записи в `ledger`. Строка scan берет `order_line_id`, `item_id`, `from_hu`, `from_location_id` и `qty` из merged-источника (plan-bound + `FILLED production_pallets`, qty = min(текущий HU stock, shipment remaining))
