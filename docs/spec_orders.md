...
- `RETURN_PLAN` запрещён, если adopted pallet уже `FILLED`, имеет component progress, ledger по этой HU/PRD, находится в закрытом target PRD либо operational composition не совпадает с provenance. Такие фактические состояния не откатываются этим workflow; их compensation относится к отдельному lifecycle slice.
