...
Для `INTERNAL`:
- `produced_qty` = сумма legacy `doc_lines.qty` по закрытым PRD без `production_pallets` + ledger-backed объем `production_pallet_lines.planned_qty` по паллетам `status = FILLED`; для CUSTOMER reserve/read-model stale HU из `order_receipt_plan_lines` с текущим ledger balance `<= 0` не считается произведенным и не показывается как обычный `production_hu_codes`
