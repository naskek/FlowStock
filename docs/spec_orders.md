...
- Открытый PRD и fill-событие не влияют на складской остаток. Успешный fill конкретной паллеты переводит паллету в `FILLED`, заполняет `filled_at`/`filled_qty` и учитывается в operational/read-model `receipt_remaining`; `ledger` пишет только canonical `Close` документа `PRODUCTION_RECEIPT`. Для palletized PRD закрытие пишет `ledger` по всем FILLED паллетам: у одиночной паллеты одна строка, у mixed pallet по одной строке на компонент. Повторный fill возвращает `already_filled` и не пишет `ledger`.
...
- При изменении количества строки заказа сервер работает только в области `order_id + order_line_id`: сбрасывает/удаляет только `PLANNED`-паллеты и строки плана, связанные с этой строкой заказа, и не трогает паллеты других строк.
...
- При уменьшении количества новое количество не может быть ниже уже отгруженного объема плюс защищенного физического покрытия строки (`FILLED`/зарезервированные HU). Нарушение блокируется сообщением `Нельзя уменьшить количество ниже уже заполненного/отгруженного объема.`
- При увеличении количества существующие `FILLED`-паллеты и валидные HU-reservations сохраняются; повторное формирование плана создает только недостающее покрытие и не дублирует уже покрытый объем.
...
- В WPF частичное или отсутствующее HU-покрытие не показывается модальным warning-окном. Строки заказа подсвечиваются: мягкий зеленый фон при полном покрытии остатка к отгрузке, мягкий розовый фон при частичном покрытии или отсутствии HU при положительном остатке; tooltip использует наименование товара и количества, а не internal `order_line_id`.
