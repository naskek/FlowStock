...
- В paged-режиме `/api/orders` сортирует заказы по операторскому приоритету: `IN_PROGRESS`, затем `ACCEPTED`, затем `DRAFT`, затем `SHIPPED`, затем `CANCELLED`; тип `CUSTOMER`/`INTERNAL` не влияет на этот приоритет. Внутри одного статуса первична ближайшая `due_date`, далее более новые `created_at/id`. Старый вызов без `limit`/`offset` сохраняет прежнюю форму ответа.
