...
- Computed `READY_HU_BINDING_AVAILABLE` показывается в WPF incoming requests как отдельный read-only тип `Готовые HU`, если computed `hu_count > 0`. Фильтр типа запроса не меняет загрузку persistent item/order requests; `ShowResolved` применяется только к ним. В Phase 4B details/double-click для этого типа только показывает информационное сообщение и не открывает карточки заказов, order-scoped HU binding или `apply-final`.
