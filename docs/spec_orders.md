...
- Отдельный отчет `Потребность производства` считается независимо от контроля минимума и не заменяет его.
  ...
  - Расчет маркировки учитывает текущую производственную потребность, открытые незакрытые `INTERNAL`-черновики/заказы и уже созданные активные/завершенные задачи маркировки.
  ...
  - Повторное создание маркировки идемпотентно: `marking_to_create = max(0, required_marking_qty - existing_active_marking_qty - existing_completed_marking_qty)`.
...
- уже созданные активные/завершенные задачи маркировки вычитаются из нового объема.
