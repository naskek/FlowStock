...
- `GET /api/orders/{orderId}/production-pallets/pre-plan-coverage-preview` является advisory preview: он показывает предупреждение о ожидаемом `INTERNAL`-выпуске и projected adoption (`adoptable_internal_planned_hus`, `adoption_skipped_candidates`, projected counts/qty), но не изменяет данные. Writer-команда `/plan` всегда повторно пересчитывает кандидатов: в начале транзакции она берёт snapshot открытых `INTERNAL` source order ids, locks этот snapshot вместе с target/customer order в порядке `order_id ASC`, затем финальную adoption projection ограничивает только этим locked source set, валидирует и применяет фактический результат только на сервере. `INTERNAL` source, появившийся после lock snapshot, не переносится в этом запуске и попадёт в следующий preview/run; WPF отправляет только `mode`, не выбранные HU и не количества.
