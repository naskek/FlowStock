...
- Если CUSTOMER order забирает `FILLED` HU из открытого INTERNAL PRD, исходный INTERNAL order не теряет объем: сервер добавляет в тот же open PRD replacement HU в статусе `PLANNED` по той же строке/товару. Старый `FILLED` HU остается историей физического выпуска и может быть отгружен CUSTOMER только при положительном stock в `ledger`; закрытие INTERNAL PRD блокируется, пока replacement HU не будет наполнен. При последующем Close PRD старый уже отгруженный HU не получает повторный приход `ledger`, а replacement HU проводится как новый физический выпуск.
- CUSTOMER OUT и TSD outbound строят ожидаемые HU только как пересечение customer reservation (`order_receipt_plan_lines`) и положительного физического HU stock из `ledger`. Reservation/bound HU без физического остатка не считается готовым к отгрузке, не попадает в TSD picking list и не должен подсвечиваться как готовый.
