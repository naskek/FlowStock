...
- Отдельный отчет `Потребность производства` считается независимо от контроля минимума и не заменяет его.
  - Для заказа используется его `due_date`; если дата не задана, строка отчета относится к сегодняшнему дню.
  ...
  - Строки отчета группируются по `Дата потребности -> Тип товара -> Номенклатура`, а для пустого типа в UI показывается `Без типа`.
