...
- `/api/orders` получил optional query params `limit`, `offset` и `include_cancelled_merged`.
...
- `DRAFT` (в т.ч. автосформированные внутренние заказы);
- `IN_PROGRESS`;
- `ACCEPTED` (`Готов`);
- `SHIPPED` (`Выполнен`);
- прочие активные статусы.

По умолчанию `CANCELLED` и `MERGED` (`Объединён`) не попадают в выборку. Для их показа используется `include_cancelled_merged=1`; тогда отменённые и объединённые идут первыми, затем активные в том же порядке.

Внутри группы сортировка стабильная: `created_at DESC`, затем `order_ref DESC`.
