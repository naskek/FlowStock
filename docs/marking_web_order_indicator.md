...
Индикатор компактный, находится в отдельной колонке `ЧЗ`, использует `title` и `aria-label`. Состояние берется только из `marking_effective_status` и `marking_status_display` API заказов; frontend не использует строки заказа, `marking_required` или raw `marking_status`.
...
- Локально запущенный PC web отдал страницу `/` со статусом 200 и подключенным `app.js`.
- В локальной БД через `/api/orders?include_internal=1` проверены заказы `PRINTED` и `NOT_REQUIRED`; заказа с `REQUIRED` в текущей локальной БД нет, поэтому REQUIRED-ветка проверена по frontend-логике `marking_effective_status === "REQUIRED"` без изменения данных.
